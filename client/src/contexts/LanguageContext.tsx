...
export function LanguageProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    const stored = (() => { try { return localStorage.getItem(LOCALE_STORAGE_KEY); } catch { return null; } })();
    if (stored) { setDetected(true); return; } // user already chose
    ...
    (async () => {
      try {
        ...
        if (res.ok) {
          ...
          if (FRENCH_PROVINCES.some(p => province.toLowerCase().includes(p.toLowerCase()))) {
            setLocale('fr');
            setDetected(true);
            return;
          }
        }
      } catch { /* geo endpoint failed — fall back */ }
