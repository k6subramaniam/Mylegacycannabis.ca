...
export default function MobileUpload() {
  ...
  const [status, setStatus] = useState<
    "ready" | "uploading" | "success" | "error" | "expired"
  >("ready");
    ...
    fetch(`/api/verify/mobile-poll/${t}`)
      ...
      .then(data => {
        if (data.status === "expired") setStatus("expired");
          ...
          setStatus("success");
          setSuccessMsg(
            "Your ID has already been submitted. You can close this page."
          );
        }
      })
  ...
  const handleFile = async (file: File) => {
    ...
    try {
      ...
      const res = await fetch("/api/verify/mobile-submit", {
        method: "POST",
        body: formData,
      });
      ...
      if (data.success) {
        setSuccessMsg(
          data.message ||
            "ID submitted! You can close this page and return to your computer."
        );
        setStatus("success");
      } else {
  ...
  return (
    <>
      <SEOHead
        title="Upload Your ID — My Legacy Cannabis"
        description="Upload your government-issued ID for age verification."
        noindex
      />
        ...
        <div className="p-5 max-w-md mx-auto">
          {status === "expired" && (
            <div className="bg-white rounded-2xl p-8 text-center shadow-sm">
              ...
              <h2 className="font-display text-lg text-[#4B2D8E] mb-2">
                SESSION EXPIRED
              </h2>
              <p className="text-sm font-body text-gray-500">
                This upload link has expired. Return to your computer and
          ...
          {status === "ready" && (
            <div className="bg-white rounded-2xl p-6 shadow-sm">
              ...
              <p className="text-sm font-body text-gray-500 mb-5">
                Take a clear photo of your driver's licence, passport, or
                provincial ID card.
              </p>
              ...
              <label className="block w-full bg-[#f0f0ec] text-[#333] font-display text-center py-4 rounded-xl cursor-pointer active:scale-[0.98] transition-transform">
                ...
                />
                <FileText size={18} className="inline mr-2 -mt-0.5" /> Choose
                from Gallery
              </label>

              <div className="flex gap-2 justify-center mt-4">
                <span className="bg-[#f0f0ec] px-3 py-1 rounded text-xs font-display text-gray-400">
                  JPG
                </span>
                <span className="bg-[#f0f0ec] px-3 py-1 rounded text-xs font-display text-gray-400">
                  PNG
              ...
              <p className="text-[10px] text-gray-400 font-body text-center mt-3">
                Your ID is stored securely and only reviewed by authorized
                staff.
              </p>
            </div>
          )}
          ...
          {status === "uploading" && (
            <div className="bg-white rounded-2xl p-8 text-center shadow-sm">
              ...
              <p className="text-sm font-body text-gray-500">
                Keep this page open while your ID is being uploaded.
              </p>
            </div>
          )}
          ...
          {status === "error" && (
            <div className="bg-white rounded-2xl p-8 text-center shadow-sm">
              ...
              <button
                ...
                className="bg-[#4B2D8E] text-white font-display py-3 px-6 rounded-xl"
              >
                Try Again
              </button>
            </div>
