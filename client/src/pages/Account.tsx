...
export default function Account() {
  ...
  return (
    <>
      <SEOHead
        title="My Account"
        description="Manage your My Legacy Cannabis account, orders, and rewards."
        noindex
      />
          ...
          <div className="bg-[#4B2D8E] rounded-2xl p-5 mb-6 flex flex-col sm:flex-row items-center justify-between gap-4">
            ...
            <div className="flex items-center gap-6">
              <div className="text-center">
                <p className="font-display text-2xl text-[#F15929]">
                  {user.rewardsPoints}
                </p>
              ...
              {idVerificationEnabled && (
                <div className="text-center">
                  <div
                    className={`w-8 h-8 rounded-full flex items-center justify-center ${user.idVerified ? "bg-green-500" : "bg-orange-500"}`}
                  >
                    <Shield size={16} className="text-white" />
                  </div>
                  <p className="text-white/70 text-[10px] font-body mt-1">
...
// ─── Shipping Address Section (within Profile tab) ──────────────────────────
function ShippingAddressSection({ user }: { user: any }) {
  const { refreshUser } = useAuth();
  const saveAddressMut = trpc.store.saveAddress.useMutation({
    onSuccess: () => {
      toast.success("Shipping address saved!");
      refreshUser();
      setEditing(false);
    },
    onError: (err: any) => toast.error(err.message || "Failed to save address"),
  });

  const clearAddressMut = trpc.store.clearAddress.useMutation({
    onSuccess: () => {
  ...
  const savedAddr: SavedAddress | null = user.savedAddress || null;
  const [editing, setEditing] = useState(false);
  const [form, setForm] = useState({
    street: savedAddr?.street || "",
    city: savedAddr?.city || "",
    province: savedAddr?.provinceCode || savedAddr?.province || "",
    postalCode: savedAddr?.postalCode || "",
  });

  // Reset form when user data changes (e.g. after save)
  useEffect(() => {
    if (!editing && savedAddr) {
      setForm({
        street: savedAddr.street || "",
        city: savedAddr.city || "",
        province: savedAddr.provinceCode || savedAddr.province || "",
  ...
  const handleAutocompleteSelect = (addr: {
    street: string;
    city: string;
    province: string;
    provinceCode: string;
  ...
  const handleSave = () => {
    ...
    const provName =
      canadianProvinces.find(p => p.code === form.province)?.name ||
      form.province;
  ...
  const handleDelete = () => {
    clearAddressMut.mutate();
  };

  const inputClass =
    "w-full bg-white rounded-lg px-4 py-3 text-sm font-body text-gray-700 border border-gray-200 focus:outline-none focus:ring-2 focus:ring-[#4B2D8E]/20 focus:border-[#4B2D8E]/40";

  // ─── Display saved address (not editing) ───
  if (savedAddr?.street && savedAddr.street.trim() && !editing) {
    return (
      <div className="bg-[#F5F5F5] rounded-2xl p-6">
        <div className="flex items-center justify-between mb-4">
          <h2 className="font-display text-lg text-[#4B2D8E] flex items-center gap-2">
            <Home size={18} /> SHIPPING ADDRESS
          </h2>
          <div className="flex items-center gap-2">
            <button
              onClick={() => setEditing(true)}
              className="flex items-center gap-1.5 px-3 py-1.5 bg-white rounded-lg text-xs font-display text-[#4B2D8E] hover:bg-[#4B2D8E]/5 border border-gray-200 transition-all"
            >
              <Edit3 size={12} /> EDIT
            </button>
            <button
              onClick={handleDelete}
              disabled={clearAddressMut.isPending}
              className="flex items-center gap-1.5 px-3 py-1.5 bg-white rounded-lg text-xs font-display text-red-500 hover:bg-red-50 border border-gray-200 transition-all disabled:opacity-50"
            >
              <Trash2 size={12} /> REMOVE
            </button>
          </div>
        </div>
        <div className="bg-white rounded-xl p-4 flex items-start gap-3 border border-gray-100">
          <MapPin size={18} className="text-[#4B2D8E] shrink-0 mt-0.5" />
          <div className="text-sm font-body text-gray-700">
            <p className="font-medium">{savedAddr.street}</p>
            <p>
              {savedAddr.city}, {savedAddr.provinceCode || savedAddr.province}{" "}
              {savedAddr.postalCode}
            </p>
        ...
        <p className="text-xs text-gray-400 font-body mt-3 flex items-center gap-1">
          ...
          This address will be available as a one-click option at checkout.
        </p>
      </div>
    );
  }
  ...
  return (
    <div className="bg-[#F5F5F5] rounded-2xl p-6">
      <div className="flex items-center justify-between mb-4">
        ...
        {editing && savedAddr?.street && savedAddr.street.trim() && (
          <button
            onClick={() => setEditing(false)}
            className="flex items-center gap-1 text-xs font-display text-gray-500 hover:text-gray-700 transition-colors"
          >
            <X size={14} /> CANCEL
          </button>
        )}
      ...
      {!savedAddr?.street || !savedAddr.street.trim() ? (
        <div className="bg-amber-50 border border-amber-200 rounded-xl p-3 mb-4 flex items-start gap-2">
          <MapPin size={14} className="text-amber-600 shrink-0 mt-0.5" />
          <p className="text-xs font-body text-amber-700">
            Add your shipping address to speed up checkout. It will appear as a
            quick-fill toggle on the checkout page.
          </p>
        </div>
      ) : null}
      ...
      <div className="mb-4">
        ...
        <p className="text-[10px] text-gray-400 font-body mt-1">
          Powered by Canada Post &middot; or fill in manually below
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        ...
        <div>
          ...
          <input
            ...
            value="Canada"
            disabled
            className="w-full bg-gray-100 rounded-lg px-4 py-3 text-sm font-body text-gray-500 cursor-not-allowed"
          />
        </div>
      </div>
      ...
      <div className="mt-5 flex items-center gap-3">
        <button
          onClick={handleSave}
          disabled={
            saveAddressMut.isPending ||
            !form.street.trim() ||
            !form.city.trim() ||
            !form.province ||
            !form.postalCode.trim()
          }
          className="flex items-center gap-2 px-5 py-2.5 bg-[#4B2D8E] text-white rounded-full font-display text-xs hover:bg-[#3a2270] disabled:opacity-50 transition-all"
        >
          {saveAddressMut.isPending ? (
            <>
        ...
        {editing && savedAddr?.street && savedAddr.street.trim() && (
          <button
            onClick={() => setEditing(false)}
            className="px-4 py-2.5 text-gray-500 font-display text-xs hover:text-gray-700 transition-colors"
          >
            CANCEL
          </button>
        )}
...
function ProfileTab({ user }: { user: any }) {
  ...
  const disabledInputClass =
    "w-full bg-gray-100 rounded-lg px-4 py-3 text-sm font-body text-gray-500 cursor-not-allowed";
      ...
      <div className="bg-amber-50 border border-amber-200 rounded-xl p-4 flex items-start gap-3">
        ...
        <div>
          <p className="text-sm font-body text-amber-800 font-medium">
            Profile details are locked after registration
          </p>
          <p className="text-xs font-body text-amber-700 mt-1">
            For security purposes, personal information cannot be changed by
            customers. If you need to update your details, please contact our
            support team.
          </p>
          <a
            href="mailto:support@mylegacycannabis.ca"
            className="inline-flex items-center gap-1.5 mt-2 text-xs font-display text-[#F15929] hover:underline"
          >
        ...
        <div className="bg-[#F5F5F5] rounded-2xl p-6">
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              <div
                className={`w-10 h-10 rounded-full flex items-center justify-center ${user.idVerified ? "bg-green-100" : "bg-orange-100"}`}
              >
                <Shield
                  size={18}
                  className={
                    user.idVerified ? "text-green-600" : "text-orange-600"
                  }
                />
              </div>
              <div>
                ...
                <p className="text-xs text-gray-500 font-body">
                  {user.idVerified
                    ...
                    : "Verify your ID to place orders."}
                </p>
              </div>
            </div>
            {!user.idVerified && (
              <Link
                ...
                className="bg-[#4B2D8E] text-white font-display text-xs py-2 px-4 rounded-full hover:bg-[#3a2270] transition-all"
              >
                {user.idVerificationStatus === "pending"
                  ? "CHECK STATUS"
                  : "VERIFY NOW"}
      ...
      {TRACKING_STEPS.map((step, i) => {
        ...
        return (
          <div key={step.key} className="flex items-center gap-1">
            ...
            />
            <span
              className={`text-[9px] ${isComplete ? "text-gray-700 font-medium" : "text-gray-400"}`}
            >
              {step.label}
            </span>
            {i < TRACKING_STEPS.length - 1 && (
              <div
                className={`w-3 h-px ${i < activeIdx ? "bg-[#4B2D8E]" : "bg-gray-300"}`}
              />
            )}
          </div>
        );
        ...
        return (
          <div key={order.id} className="bg-[#F5F5F5] rounded-2xl p-5">
            <div className="flex items-center justify-between mb-3">
              ...
              <span
                className={`px-3 py-1 rounded-full text-xs font-display ${statusColors[order.status] || ""}`}
              >
            ...
            <div className="space-y-2 mb-3">
              {order.items.map((item: any, i: number) => (
                <div
                  key={i}
                  className="flex items-center justify-between text-sm font-body gap-2"
                >
                  <div className="flex items-center gap-2 min-w-0">
                    {item.productSlug ? (
                      <Link
                        href={`/product/${item.productSlug}`}
                        className="text-[#4B2D8E] hover:text-[#F15929] underline underline-offset-2 decoration-[#4B2D8E]/30 hover:decoration-[#F15929]/50 transition-colors truncate font-medium"
                      >
                  ...
                  <span className="font-mono-legacy text-[#333] shrink-0">
            ...
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between border-t border-gray-200 pt-3 gap-2">
              <span className="font-display text-sm text-[#4B2D8E]">
    ...
    <div className="space-y-6">
      ...
      <div className="bg-[#4B2D8E] rounded-2xl p-6 text-center text-white relative overflow-hidden">
        ...
        <p className="text-xs text-white/50 font-body mt-2">
          Earn {POINTS_PER_DOLLAR} point for every $1 spent. Redeem at checkout.
        </p>
      </div>
      ...
      <div className="bg-[#F5F5F5] rounded-2xl p-6">
        ...
        <p className="text-sm text-gray-600 font-body mb-3">
          Share your referral code or link and earn {REFERRAL_BONUS_REFERRER}{" "}
          points when your friend signs up!
        </p>
        <div className="flex items-center gap-2 mb-2">
          <div className="flex-1 bg-white rounded-lg px-4 py-3 font-mono-legacy text-sm text-[#4B2D8E] font-medium">
            {user.referralCode}
          </div>
          <button
            onClick={() => {
            ...
            className="bg-[#F15929] text-white p-3 rounded-lg hover:bg-[#d94d22] transition-all"
      ...
      {/* History */}
      <div className="bg-[#F5F5F5] rounded-2xl p-6">
        <h3 className="font-display text-lg text-[#4B2D8E] mb-4">
          POINTS HISTORY
        </h3>
        <div className="space-y-3 max-h-96 overflow-y-auto">
          {user.rewardsHistory.map((entry: any) => (
            ...
            >
              <div className="flex items-center gap-3">
                <div
                  className={`w-8 h-8 rounded-full flex items-center justify-center ${entry.type === "earned" ? "bg-green-100" : entry.type === "redeemed" ? "bg-red-100" : "bg-[#4B2D8E]/10"}`}
                >
                  {entry.type === "earned" ? (
                    ...
                    <Gift size={14} className="text-[#4B2D8E]" />
                  )}
                </div>
                <div>
                  <p className="text-xs font-body text-[#333]">
                    {entry.description}
                  </p>
              ...
              <span
                className={`font-mono-legacy text-sm font-medium ${entry.points > 0 ? "text-green-600" : "text-red-600"}`}
              >
                {entry.points > 0 ? "+" : ""}
                {entry.points}
              </span>
