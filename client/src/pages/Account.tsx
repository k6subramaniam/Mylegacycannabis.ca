...
import { POINTS_PER_DOLLAR, REFERRAL_BONUS_REFERRER, canadianProvinces } from '@/lib/data';
import { User, Package, Gift, Shield, LogOut, Copy, Star, Lock, Mail, MessageSquare, Share2, Loader2, Truck, MapPin, Edit3, Trash2, Check, Home, X } from 'lucide-react';
...
function ShippingAddressSection({ user }: { user: any }) {
  ...
  const handleAutocompleteSelect = (addr: { street: string; city: string; province: string; provinceCode: string; postalCode: string }) => {
    setForm({
      street: addr.street,
      city: addr.city,
  ...
  const handleSave = () => {
    ...
    const provName = canadianProvinces.find(p => p.code === form.province)?.name || form.province;
  ...
  const inputClass = 'w-full bg-white rounded-lg px-4 py-3 text-sm font-body text-gray-700 border border-gray-200 focus:outline-none focus:ring-2 focus:ring-[#4B2D8E]/20 focus:border-[#4B2D8E]/40';
      ...
      <div className="bg-[#F5F5F5] rounded-2xl p-6">
        ...
        <div className="bg-white rounded-xl p-4 flex items-start gap-3 border border-gray-100">
          ...
          <div className="text-sm font-body text-gray-700">
            ...
            <p>{savedAddr.city}, {savedAddr.provinceCode || savedAddr.province} {savedAddr.postalCode}</p>
  ...
  return (
    <div className="bg-[#F5F5F5] rounded-2xl p-6">
      ...
      {!savedAddr?.street || !savedAddr.street.trim() ? (
        <div className="bg-amber-50 border border-amber-200 rounded-xl p-3 mb-4 flex items-start gap-2">
          ...
          <p className="text-xs font-body text-amber-700">
            Add your shipping address to speed up checkout. It will appear as a quick-fill toggle on the checkout page.
          </p>
        </div>
      ) : null}
      ...
      <div className="mb-4">
        <label className="text-xs text-gray-500 font-body block mb-1">Search Address</label>
        <AddressAutocomplete
          onSelect={handleAutocompleteSelect}
          onClear={() => setForm({ street: '', city: '', province: '', postalCode: '' })}
          defaultValue={form.street}
          placeholder="Start typing your address..."
        />
        <p className="text-[10px] text-gray-400 font-body mt-1">Powered by Canada Post &middot; or fill in manually below</p>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        ...
        <div>
          <label className="text-xs text-gray-500 font-body block mb-1">Country</label>
          <input type="text" value="Canada" disabled className="w-full bg-gray-100 rounded-lg px-4 py-3 text-sm font-body text-gray-500 cursor-not-allowed" />
        </div>
      </div>
      ...
      <div className="mt-5 flex items-center gap-3">
        <button
          onClick={handleSave}
          disabled={saveAddressMut.isPending || !form.street.trim() || !form.city.trim() || !form.province || !form.postalCode.trim()}
