...
const HERO_IMG =
  "https://d2xsxph8kpxj0f.cloudfront.net/86973655/5wgxseZemq4jvbSSj7t6zG/hero-shop-5tiqFdCHUdeMeR3zPVXYu5.webp";
...
const GRADE_CATEGORIES = new Set([
  "flower",
  "pre-rolls",
  "ounce-deals",
  "shake-n-bake",
...
export default function Shop() {
    ...
    if (shouldGroup) {
      ...
      for (const [key, variants] of Array.from(groupMap.entries())) {
        ...
        groups.push({
          ...
          baseName: key.startsWith("__single_")
            ? variants[0].name
            : deriveBaseName(variants[0].name),
          variants,
          fromPrice: minPrice,
          priceLabel,
        });
  ...
  const sorted = useMemo(() => {
    ...
    switch (sortBy) {
      ...
      case "potency": {
        ...
        result.sort(
          (a, b) => parseTHC(b.product.thc) - parseTHC(a.product.thc)
        );
        break;
      }
      default:
        result.sort(
          (a, b) => (b.product.featured ? 1 : 0) - (a.product.featured ? 1 : 0)
        );
    }
    return result;
  }, [grouped, sortBy]);
  ...
  return (
    <>
      <SEOHead
        title={
          selectedCategory && CATEGORY_SEO[selectedCategory]
        ...
        jsonLd={buildBreadcrumbJsonLd([
          ...
          ...(activeCat
            ? [
                {
                  name: activeCat.name,
                  url: canonical(`/shop/${selectedCategory}`),
                },
              ]
            : []),
        ])}
        ...
        <section className="bg-white border-b border-gray-200">
          <div className="container">
            <div className="flex items-center gap-2 overflow-x-auto py-2.5 scrollbar-hide -mx-1 px-1">
              <span className="shrink-0 text-[11px] font-display text-gray-400 mr-1">
                GRADE:
              </span>
              <button
              ...
              {GRADE_FILTERS.map(grade => (
                <button
                  ...
                  className={`shrink-0 px-3.5 py-1.5 rounded-full font-display text-[11px] transition-all whitespace-nowrap ${
                    selectedGrade === grade
                      ? (GRADE_COLORS[grade] || "bg-[#4B2D8E] text-white") +
                        " shadow-sm"
            ...
            <div className="flex items-center gap-2 overflow-x-auto py-2.5 scrollbar-hide -mx-1 px-1">
              ...
              {STRAIN_TYPES.map(strain => (
                <button
                  key={strain}
                  onClick={() =>
                    setSelectedStrain(selectedStrain === strain ? "" : strain)
                  }
                  className={`shrink-0 px-3.5 py-1.5 rounded-full font-display text-[11px] transition-all whitespace-nowrap ${
      ...
      <section className="bg-white py-6 md:py-10">
        <div className="container">
          ...
          <div className="flex flex-wrap items-center gap-3 mb-6">
            <div className="relative">
              ...
              >
                ...
                {isFlowerCategory && (
                  <option value="potency">POTENCY: STRONGEST FIRST</option>
                )}
              </select>
              <ChevronDown
                size={14}
          ...
          {(selectedCategory === "ounce-deals" ||
            selectedCategory === "shake-n-bake") && (
            <div className="bg-[#4B2D8E]/5 border border-[#4B2D8E]/10 rounded-xl p-3 mb-5 text-center">
              <p className="text-sm font-body text-[#4B2D8E]">
                *Mix and match available within same category/pricing
              </p>
            </div>
          )}
                      ...
                      >
                          ...
                          <div className="relative aspect-square bg-[#F5F5F5] overflow-hidden">
                            ...
                            <div className="absolute top-3 left-3 flex flex-col gap-1.5">
                              ...
                              {(product as any).grade && (
                                <span
                                  className={`font-display text-[10px] px-2.5 py-1 rounded-full ${GRADE_COLORS[(product as any).grade] || "bg-gray-400 text-white"}`}
                                >
                                  {(product as any).grade}
                                </span>
                              )}
                        ...
                        <div className="p-4">
                          ...
                          <div className="flex items-center justify-between">
                            ...
                            <div>
                              {hasVariants && (
                                <span className="text-[10px] text-gray-400 font-display block">
                                  FROM
                                </span>
                              )}
                          ...
                          <p className="text-[10px] text-[#4B2D8E] font-body mt-2 flex items-center gap-1">
                            ...
                            {group.fromPrice.toFixed(0)} points
                          </p>
                        </div>
                      </div>
                    );
