...
export default function ProductPage() {
  ...
  const { data: product, isLoading } = trpc.store.product.useQuery({
    slug: slug || "",
  });
  ...
  const sortedVariants = useMemo(() => {
    ...
    return [...variants].sort(
      (a: any, b: any) => parseWeight(a.weight) - parseWeight(b.weight)
    );
  }, [variants]);
  ...
  if (!product) {
    return (
      <div className="container py-20 text-center">
        <h1 className="font-display text-2xl text-[#4B2D8E] mb-4">
          {t.productPage.notFound}
        </h1>
        <Link
          href="/shop"
          className="text-[#F19929] font-display hover:underline"
        >
  ...
  const points = calculatePointsEarned(
    parseFloat(ap.price?.toString() || "0") * quantity
  );
  ...
  const productSchema = {
    ...
    image: product.image ? [product.image] : [`${SITE_URL}/logo.webp`],
    sku: `MLC-${product.id}`,
  ...
  const handleAddToCart = () => {
    ...
    trackAddToCart(ap.slug, ap.id, {
      quantity,
      price: ap.price,
      name: ap.name,
    });
    toast.success(
      interpolate(t.productPage.addedToCartToast, { quantity, name: ap.name })
    );
    setQuantity(1);
  };
    ...
    <>
      <SEOHead
        ...
        description={
          ...
          product.description ||
          `Shop ${baseName} at My Legacy Cannabis. Premium cannabis products with free shipping over $150.`
        }
        ...
        jsonLd={[
          ...
          buildBreadcrumbJsonLd([
            ...
            { name: "Shop", url: canonical("/shop") },
            {
              ...
              url: canonical(
                `/shop/${product.categorySlug || product.category?.toLowerCase().replace(/\s+/g, "-")}`
              ),
            },
          ...
          <Breadcrumbs
            items={[
              ...
              { label: t.common.shop, href: "/shop" },
              {
                ...
                href: `/shop/${(product as any).categorySlug || product.category.toLowerCase().replace(/\s+/g, "-")}`,
              },
              { label: baseName },
            ]}
          />
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mt-4">
            ...
            <div className="flex items-center justify-center bg-[#F5F5F5] rounded-2xl aspect-square overflow-hidden">
              <img
                src={
                  ap.image ||
                  "https://images.unsplash.com/photo-1599599810694-b5ac4dd64b74?w=600"
                }
                alt={baseName}
            ...
            <div>
              ...
              <div className="mb-4">
                <div className="flex items-center gap-2 mb-3 flex-wrap">
                  ...
                  {product.strainType && product.strainType !== "N/A" && (
                    <span className="inline-block bg-[#F19929] text-white font-display text-xs px-3 py-1 rounded-full">
                      {product.strainType}
                    </span>
                  )}
                  {(product as any).grade && (
                    <span
                      className={`inline-block font-display text-xs px-3 py-1 rounded-full ${GRADE_COLORS[(product as any).grade] || "bg-[#4B2D8E] text-white"}`}
                    >
                      {(product as any).grade}
                    </span>
                  )}
                </div>
                <h1 className="font-display text-4xl md:text-5xl text-[#4B2D8E] mb-2">
                  {baseName.toUpperCase()}
                </h1>
                <p className="text-gray-600 font-body">
                  {product.flavor && product.flavor !== (product as any).grade
                    ? `${product.flavor} `
                    : ""}
                  {ap.weight && `• ${ap.weight}`}
                </p>
              ...
              {hasWeightVariants && (
                <div className="mb-6 pb-6 border-b border-gray-200">
                  <label className="block font-display text-sm text-[#4B2D8E] mb-3">
                    SELECT SIZE
                  </label>
              ...
              <div className="mb-6 pb-6 border-b border-gray-200">
                <div className="flex items-baseline gap-2 mb-2">
                  <span className="font-display text-4xl text-[#4B2D8E]">
                    ${priceNum}
                  </span>
                  {hasWeightVariants && (
                  ...
                  <span className="text-sm text-gray-500 font-body">
                    THC: {ap.thc || product.thc}
                  </span>
                </div>
                <p className="text-[#F19929] font-display text-sm">
                  {interpolate(t.productPage.earnPointsWithPurchase, {
                    points,
                  })}
                </p>
              </div>
              ...
              <div className="mb-6 pb-6 border-b border-gray-200">
                <h3 className="font-display text-sm text-[#4B2D8E] mb-2">
                  {t.productPage.aboutThisProduct}
                </h3>
              ...
              <div className="mb-6">
                <label
                  ...
                  className="block font-display text-sm text-[#4B2D8E] mb-3"
                >
              ...
              <div className="space-y-3 pt-6 border-t border-gray-200">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-gray-500 font-body">
                      {interpolate(t.productPage.freeShippingDesc, {
                        threshold: String(FREE_SHIPPING_THRESHOLD),
                      })}
                    </p>
                  </div>
                </div>
                {idVerificationEnabled && (
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="font-display text-xs text-[#4B2D8E]">
                        {t.productPage.ageVerified}
                      </p>
                      <p className="text-xs text-gray-500 font-body">
                        {t.productPage.ageVerifiedDesc}
                      </p>
                    </div>
                  </div>
                )}
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-gray-500 font-body">
                      {t.productPage.fastDeliveryDesc}
                    </p>
                  </div>
                </div>
              </div>
  ...
  return (
    <div className="mt-12 pt-10 border-t border-gray-200">
      <div className="flex items-center justify-between mb-6">
        <h2 className="font-display text-2xl text-[#4B2D8E]">
          CUSTOMERS ALSO BOUGHT
        </h2>
        <Link
          href={`/shop/${category}`}
          className="text-sm text-[#F15929] hover:underline font-display flex items-center gap-1"
        >
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {related.map((p: any) => (
          <div
            key={p.id}
            className="bg-white rounded-2xl overflow-hidden group hover:shadow-xl transition-shadow border border-gray-100"
          >
            <Link href={`/product/${p.slug}`} className="block">
              <div className="relative aspect-square bg-[#F5F5F5] overflow-hidden">
                ...
                />
                {p.featured && (
                  <span className="absolute top-2 left-2 bg-[#4B2D8E] text-white font-display text-[9px] px-2 py-0.5 rounded-full">
                    ⭐ STAFF PICK
                  </span>
                )}
                {p.stock > 0 && p.stock <= 5 && (
                  <span className="absolute top-2 right-2 bg-red-500 text-white font-display text-[9px] px-2 py-0.5 rounded-full animate-pulse">
                    LOW STOCK
                  </span>
                )}
              </div>
            ...
            <div className="p-3">
              ...
              <p className="text-[10px] text-gray-500 font-body mb-2">
                {p.weight} {p.thc ? `• THC: ${p.thc}` : ""}
              </p>
