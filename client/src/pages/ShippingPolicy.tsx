        ...
        <div className="container max-w-4xl">
          ...
          <div className="bg-[#F15929] rounded-2xl p-6 text-white text-center mb-8">
            ...
            <h2 className="font-display text-2xl mb-1">
              {t.shippingPage.freeShipping}
            </h2>
            <p className="font-body text-white/90">
              {t.shippingPage.freeShippingDesc.replace(
                "{threshold}",
          ...
          <div className="mb-8">
            ...
            <div className="bg-[#F5F5F5] rounded-2xl overflow-hidden">
              <div className="grid grid-cols-3 bg-[#4B2D8E] text-white font-display text-xs p-4">
                <span>{t.shippingPage.region}</span>
                <span>{t.shippingPage.rate}</span>
                <span>{t.shippingPage.deliveryTime}</span>
              </div>
              {shippingZones.map((zone, i) => (
                <div
                  key={zone.name}
                  className={`grid grid-cols-3 p-4 text-sm font-body ${i % 2 === 0 ? "bg-white" : "bg-[#F5F5F5]"}`}
                >
                  <div>
                  ...
                  <span className="font-mono-legacy text-[#4B2D8E] font-medium">
                    ${zone.rate.toFixed(2)}
                  </span>
          ...
          <div className="space-y-6">
            <h2 className="font-display text-2xl text-[#4B2D8E]">
              {t.shippingPage.shippingDetails}
            </h2>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {[
                {
                  icon: Package,
                  title: t.shippingPage.carrier,
                  desc: t.shippingPage.carrierDesc,
                },
                {
                  icon: Clock,
                  title: t.shippingPage.processingTime,
                  desc: t.shippingPage.processingTimeDesc,
                },
                {
                  icon: Shield,
                  title: t.shippingPage.discreetPackaging,
                  desc: t.shippingPage.discreetPackagingDesc,
                },
                {
                  icon: MapPin,
                  title: t.shippingPage.deliveryArea,
                  desc: t.shippingPage.deliveryAreaDesc,
                },
              ].map((item, i) => (
                <div
                  key={i}
                  className="bg-[#F5F5F5] rounded-xl p-5 flex items-start gap-4"
                >
                  <div className="w-10 h-10 rounded-full bg-[#4B2D8E] flex items-center justify-center shrink-0">
                    <item.icon size={18} className="text-white" />
                  </div>
                  <div>
                    <h3 className="font-display text-sm text-[#4B2D8E]">
                      {item.title}
                    </h3>
              ...
              <ul className="space-y-3">
                {[
                  ...
                  >
                    <CheckCircle
                      size={16}
                      className="text-[#F15929] shrink-0 mt-0.5"
                    />{" "}
                    {item}
                  </li>
                ))}
            ...
            <div className="bg-orange-50 border border-orange-200 rounded-xl p-5">
              <div className="flex items-start gap-3">
                ...
                />
                <div>
                  <h3 className="font-display text-sm text-orange-700">
                    {t.shippingPage.ageVerificationRequired}
                  </h3>
                  <p className="text-sm text-orange-600 font-body mt-1">
                    {t.shippingPage.ageVerificationDesc}
                  </p>
                </div>
              </div>
            </div>
