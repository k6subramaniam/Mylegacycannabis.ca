    ...
    return (
      <>
        ...
        <section className="container py-20 text-center">
          ...
          <Link
            href="/shop"
            className="inline-flex items-center gap-2 bg-[#F15929] text-white font-display py-3 px-8 rounded-full hover:bg-[#d94d22] transition-all"
          >
        ...
        <div className="container">
          ...
          <div className="bg-[#F5F5F5] rounded-xl p-4 mb-6">
            {isFreeShipping ? (
              <p className="text-sm font-body text-[#4B2D8E] flex items-center gap-2">
                <Truck size={16} className="text-[#F15929]" />{" "}
                <strong>{t.cart.youQualifyFreeShipping}</strong>
              </p>
            ) : (
              <>
                <p className="text-sm font-body text-[#333] mb-2">
                ...
                <div className="w-full bg-gray-200 rounded-full h-2.5">
                  <div
                    className="bg-[#F15929] h-2.5 rounded-full transition-all"
          ...
          {user && (
            <div className="bg-[#4B2D8E]/5 border border-[#4B2D8E]/10 rounded-xl p-4 mb-6 flex items-center justify-between flex-wrap gap-3">
              <p className="text-sm font-body text-[#4B2D8E] flex items-center gap-2">
                <Gift size={16} className="text-[#F15929]" /> You have{" "}
                <strong>{user.rewardsPoints} points</strong> available
              </p>
                  ...
                  {eligibleTiers.map(tier => (
                    <button
                      ...
                      onClick={() => {
                        applyReward(tier);
                        toast.success(
                          `${tier.name} reward applied — $${tier.discount} OFF`
                        );
                      }}
              ...
              {appliedReward && (
                <div className="flex items-center gap-2">
                  <span className="text-sm font-body text-[#F15929] font-medium">
                    {appliedReward.name}: -${rewardDiscount.toFixed(2)}
                  </span>
                  <button
            ...
            <div className="lg:col-span-2 space-y-4">
              {items.map(item => (
                <motion.div
                  key={item.product.id}
                  layout
                  className="bg-[#F5F5F5] rounded-xl p-4 flex gap-4"
                >
                  <Link
                    ...
                    <div className="flex items-start justify-between gap-2">
                      <div>
                        <Link href={`/product/${item.product.slug}`}>
                          <h3 className="font-display text-sm text-[#4B2D8E] hover:text-[#F15929] transition-colors">
                      ...
                      <button
                        onClick={() => {
                          ...
                          trackRemoveFromCart(
                            item.product.slug,
                            Number(item.product.id) || undefined
                          );
                    ...
                    <div className="flex items-center justify-between mt-3">
                      <div className="flex items-center bg-white rounded-full">
                        <button
                          ...
                          className="p-2 hover:text-[#F15929]"
                          aria-label="Decrease"
                        >
                          <Minus size={14} />
                        </button>
                        <span className="w-8 text-center text-sm font-display">
                          {item.quantity}
                        </span>
                        <button
                          onClick={() =>
              ...
              <div className="bg-[#F5F5F5] rounded-2xl p-6 sticky top-28">
                ...
                <div className="mb-4">
                  ...
                  <select
                    ...
                    className="w-full bg-white rounded-lg px-3 py-2.5 text-sm font-body border-none focus:ring-2 focus:ring-[#4B2D8E]"
                ...
                <div className="space-y-3 text-sm font-body border-b border-gray-200 pb-4 mb-4">
                  ...
                  {rewardDiscount > 0 && (
                    <div className="flex justify-between text-[#F15929]">
                      <span>{t.cart.rewardsDiscount}</span>
                  ...
                  <div className="flex justify-between">
                    ...
                    <span className="text-[#333]">$0.00</span>
                  </div>
                </div>
                ...
                <p className="text-xs text-[#4B2D8E] font-body mb-4 flex items-center gap-1">
                  <Gift size={12} className="text-[#F15929]" /> You'll earn{" "}
                  <strong>{pointsToEarn} points</strong> with this order
                </p>
                ...
                {!meetsMinimum && (
                  <div className="bg-red-50 border border-red-200 rounded-lg p-3 mb-4 flex items-start gap-2">
                    ...
                    <p className="text-xs text-red-600 font-body">
                      ...
                      {(MINIMUM_ORDER - subtotal).toFixed(2)} more to proceed.
                    </p>
                  </div>
                )}
                    ...
                    <p className="text-xs font-body text-[#4B2D8E] flex items-start gap-1.5">
                      ...
                      <span>
                        {user?.idVerificationStatus === "approved" ? (
                          ...
                          "ID verification required at checkout (19+)"
                        )}
                      </span>
                    </p>
                  </div>
                ...
                <Link
                  href="/shop"
                  className="block text-center text-sm text-[#4B2D8E] hover:text-[#F15929] font-body mt-3 transition-colors"
                >
