...
export default function IDVerification() {
  const { user, isAuthenticated, submitIdVerification, updateProfile } =
    useAuth();
  ...
  useEffect(() => {
    ...
    return () => {
      /* leave cached — no need to remove */
    };
  }, []);
    ...
    fetch(`/api/verify/check?email=${encodeURIComponent(email)}`)
      ...
      .then(data => {
        if (data.id && data.status) {
          ...
          setSubmittedStatus(
            data.status === "pending" ? "pending_review" : data.status
          );
    ...
    return (
      <>
        <SEOHead
          ...
          description="ID verification is not required at this time."
          noindex
        />
        <section className="container py-12">
          <Breadcrumbs
            items={[{ label: "Home", href: "/" }, { label: "ID Verification" }]}
          />
          <div className="max-w-lg mx-auto text-center">
            ...
            <p className="text-gray-600 font-body mb-6">
              ID verification is not currently required. You can place orders
              directly.
            </p>
            <Link
              href="/shop"
  ...
  if ((isAuthenticated && user?.idVerified) || submittedStatus === "approved") {
    return (
      <>
        <SEOHead
          title="ID Verified"
          description="Your identity has been verified. You can now place orders."
          noindex
        />
        <section className="container py-12">
          <Breadcrumbs
          ...
          <div className="max-w-lg mx-auto text-center">
            ...
            {isAuthenticated && (
              <p className="text-sm text-gray-400 font-body mb-6">
                This is a one-time verification — you won't need to verify
                again.
              </p>
            )}
            <Link
    ...
    return (
      <>
        <SEOHead
          ...
          description="Your ID verification is under review. We will notify you once approved."
          noindex
        />
        <section className="container py-12">
          <Breadcrumbs
          ...
          <div className="max-w-lg mx-auto text-center">
            ...
            <div className="bg-[#F5F5F5] rounded-xl p-5 text-left mb-6">
              <h3 className="font-display text-xs text-gray-500 mb-4 tracking-wider">
                WHAT HAPPENS NEXT
              </h3>
              <div className="space-y-4">
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    <p className="text-sm font-body font-semibold text-[#333]">
                      ID Uploaded
                    </p>
                    <p className="text-xs text-gray-400 font-body">
                      Document securely received
                    </p>
                  </div>
                </div>
                <div className="flex items-start gap-3">
            ...
            <Link
              href="/shop"
              className="text-[#4B2D8E] font-display text-sm hover:text-[#F15929]"
            >
              ← CONTINUE BROWSING
            </Link>
          </div>
  ...
  if (
    ...
    (isAuthenticated && user?.idVerificationStatus === "rejected")
  ) {
    return (
      <>
        <SEOHead
          title="ID Verification — Rejected"
          description="Your ID verification was not approved. Please resubmit with a valid government-issued photo ID."
          noindex
        />
        <section className="container py-12">
          <Breadcrumbs
          ...
          <div className="max-w-lg mx-auto text-center">
            ...
            <p className="text-gray-600 font-body mb-6">
              We were unable to verify your ID. Please try again with a clear,
              well-lit photo.
            </p>
            <button
              onClick={() => {
                ...
                setFrontFile(null);
                if (isAuthenticated)
                  updateProfile({ idVerificationStatus: "none" });
              }}
      ...
      try {
        ...
        if (data.status === "submitted") {
          ...
          if (isAuthenticated)
            updateProfile({ idVerificationStatus: "pending" });
          toast.success(
            "ID uploaded from your phone and submitted for review!"
          );
  ...
  return (
    <>
      <SEOHead
        ...
        description="Upload your government-issued ID to verify your age (19+)."
        noindex
      />
      <section className="container py-6 md:py-10">
        ...
        <div className="max-w-2xl mx-auto">
          <div className="flex items-center gap-3 mb-2">
            ...
            <h1 className="font-display text-2xl md:text-3xl text-[#4B2D8E]">
              VERIFY YOUR ID
            </h1>
          </div>
          <p className="text-gray-600 font-body mb-6">
            Canadian law requires you to be 19+ to purchase cannabis. Upload a
            valid government-issued photo ID for review by our team.
          </p>

          {/* Guest email */}
          {!isAuthenticated && (
            <div className="bg-[#4B2D8E]/5 border border-[#4B2D8E]/10 rounded-xl p-4 mb-6">
              <label className="font-display text-sm text-[#4B2D8E] block mb-1">
                YOUR EMAIL ADDRESS
              </label>
              <p className="text-xs text-gray-500 font-body mb-3">
                We'll email you when your ID has been reviewed so you can return
              ...
              <p className="text-xs text-gray-400 font-body mt-2">
                ...
                <Link
                  href="/account/login"
                  className="text-[#4B2D8E] hover:underline font-semibold"
                >
                  Sign in
                </Link>{" "}
          ...
          <div className="bg-[#F5F5F5] rounded-xl p-4 mb-6">
            ...
            <ul className="text-sm font-body text-gray-600 space-y-1">
              <li className="flex items-center gap-2">
                <CheckCircle size={14} className="text-green-500" /> Canadian
                Driver's Licence
              </li>
              <li className="flex items-center gap-2">
                <CheckCircle size={14} className="text-green-500" /> Canadian
              ...
              <li className="flex items-center gap-2">
                <CheckCircle size={14} className="text-green-500" /> Provincial
                ID Card
              </li>
            </ul>
          </div>
          ...
          {/* Upload area */}
          <div className="mb-6">
            <label className="font-display text-sm text-[#4B2D8E] mb-2 block">
              GOVERNMENT-ISSUED ID *
            </label>
            <label
              className={`block border-2 border-dashed rounded-2xl p-8 text-center cursor-pointer transition-all ${frontFile ? "border-green-400 bg-green-50" : "border-gray-300 hover:border-[#4B2D8E] bg-[#F5F5F5]"}`}
            >
              <input
                type="file"
          ...
          <div className="hidden md:block mb-6">
            <div className="flex items-center gap-3 mb-3">
              ...
              <span className="text-xs font-display text-gray-400 tracking-widest">
                OR USE YOUR PHONE
              </span>
              <div className="flex-1 h-px bg-gray-200" />
            </div>
            <div className="bg-[#F5F5F5] border border-gray-200 rounded-xl p-5 text-center">
              ...
              <p className="text-xs text-gray-500 font-body mb-4">
                Scan this QR code with your phone to open the camera and
                photograph your ID directly.
              </p>
              {!qrActive ? (
                <button
                  onClick={generateQR}
                ...
                <div>
                  ...
                  <p className="text-xs text-gray-400 font-body mb-2 break-all">
                    {qrUrl}
                  </p>
                  ...
                  <button
                    onClick={generateQR}
                    className="text-xs text-[#4B2D8E] hover:underline font-body mt-2 inline-block"
                  >
                    Generate a new code
                  </button>
                </div>
          ...
          <div className="bg-[#F5F5F5] rounded-xl p-4 mb-6 flex gap-3">
            ...
            <p className="text-xs text-gray-500 font-body">
              Your ID is stored securely and only reviewed by authorized staff.
          ...
          <button
            ...
            disabled={
              !frontFile || submitting || (!isAuthenticated && !guestEmail)
            }
