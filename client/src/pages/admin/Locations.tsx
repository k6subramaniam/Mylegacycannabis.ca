...
export default function AdminLocations() {
  ...
  const { data: locations, isLoading } = trpc.admin.locations.list.useQuery(
    undefined,
  ...
  const createMutation = trpc.admin.locations.create.useMutation({
    onSuccess: () => {
      utils.admin.locations.list.invalidate();
  ...
  const handleSaveEdit = () => {
    if (
      ...
      toast.error("Name, address, city, and phone are required");
      return;
    }
    ...
    <div className="p-4 md:p-6 space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500">
            Manage your physical store locations shown on the website
          </p>
        </div>
        <button
          onClick={() => {
            setShowNew(true);
            setNewForm({ ...emptyForm, sortOrder: locations?.length || 0 });
          }}
      ...
      {showNew && (
        <div className="bg-white border-2 border-[#4B2D8E] rounded-xl p-5 space-y-4">
          <div className="flex items-center justify-between">
            <h2 className="font-semibold text-[#4B2D8E] flex items-center gap-2">
          ...
          <div className="flex gap-2 pt-2">
            <button
              ...
              className="flex items-center gap-2 bg-[#4B2D8E] hover:bg-[#3a2270] text-white px-4 py-2 rounded-lg text-sm font-medium disabled:opacity-50"
            >
              <Save size={14} />{" "}
      ...
      {isLoading ? (
        ...
        <div className="space-y-4">
          {locations.map((loc: any) => (
            <div
              key={loc.id}
              className={`bg-white rounded-xl border shadow-sm overflow-hidden transition-all ${!loc.isActive ? "opacity-60" : ""}`}
            >
              {editingId === loc.id ? (
                /* Edit mode */
                <div className="p-5 space-y-4">
                  <div className="flex items-center justify-between">
                    <h3 className="font-semibold text-[#4B2D8E] flex items-center gap-2">
                      <Edit2 size={16} /> Editing: {loc.name}
                    </h3>
                    <button
                  ...
                  <div className="flex items-start justify-between gap-4">
                    <div className="flex items-start gap-3 min-w-0 flex-1">
                      ...
                      <div className="min-w-0">
                        ...
                        {loc.mapUrl && (
                          <p
                            className="text-xs text-blue-500 mt-1 truncate max-w-md"
                            title={loc.mapUrl}
                          >
                            Map: {loc.mapUrl.substring(0, 60)}...
                          </p>
                        )}
                      </div>
                    </div>
      ...
      <div className="bg-blue-50 border border-blue-200 rounded-xl p-4 text-sm text-blue-700">
        ...
        <ul className="list-disc ml-4 space-y-0.5 text-blue-600">
          <li>
            Active locations appear on the public Locations page and homepage
          </li>
          <li>
            Use <strong>Sort Order</strong> to control display order (lower
            numbers appear first)
          </li>
          <li>
            The <strong>Map Embed URL</strong> should be a Google Maps embed
            link (iframe src)
          </li>
          <li>
            The <strong>Directions URL</strong> is the Google Maps link for "Get
            Directions"
          </li>
          <li>
            Toggle visibility with the eye icon without deleting the location
          </li>
        </ul>
      </div>
    </div>
...
function LocationFormFields({
  ...
  const update = (field: keyof LocationForm, value: any) =>
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
      <div>
        ...
        <input
          ...
          className="w-full border rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-[#4B2D8E]/30 focus:border-[#4B2D8E] outline-none"
        />
      </div>
      <div>
        <label className="block text-xs font-medium text-gray-600 mb-1">
      ...
      <div className="md:col-span-2">
        ...
        <input
          ...
          className="w-full border rounded-lg px-3 py-2 text-sm text-gray-500 focus:ring-2 focus:ring-[#4B2D8E]/30 focus:border-[#4B2D8E] outline-none"
        />
      </div>
      <div className="md:col-span-2">
        <label className="block text-xs font-medium text-gray-600 mb-1">
