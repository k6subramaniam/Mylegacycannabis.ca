...
export default function MenuImport() {
    ...
    async (file: File) => {
      ...
      try {
        ...
        if (res.success && res.items.length > 0) {
          ...
          setParseError(
            res.error || "No products found. Please try a clearer image."
          );
  ...
  const priceTierCount = items
    .filter(i => i.include)
    .reduce(
      (sum, item) => sum + Object.values(item.prices).filter(p => p).length,
      0
    );
    ...
    <div className="p-4 md:p-6 space-y-6 max-w-6xl">
      <div>
        ...
        <p className="text-sm text-gray-500 mt-1">
          Upload a photo of your cannabis menu to automatically update your
          product list for Nationwide Shipping
        </p>
      </div>
        ...
        <div className="space-y-4">
          ...
          >
            {parseMutation.isPending ? (
              <div className="space-y-4">
                ...
                <p className="text-lg font-medium text-[#4B2D8E]">
                  Analyzing your menu...
                </p>
                <p className="text-sm text-gray-500">
                  AI is reading every strain, grade, THC level, and price from
                  your menu photo. This typically takes 10-20 seconds.
                </p>
              </div>
            ) : (
              <div className="space-y-4">
          ...
          <div className="bg-blue-50 border border-blue-200 rounded-xl p-4">
            ...
            <ol className="text-sm text-blue-700 space-y-1 list-decimal list-inside">
              <li>
                Upload a photo of your cannabis menu (like the one from your
                supplier)
              </li>
              <li>
                AI extracts every strain, grade, THC %, and all price tiers (1g
                to 28g)
              </li>
              <li>
                Review the extracted data, set stock quantities for each item
              </li>
              <li>
                Click "Import" to update your Nationwide Shipping product list
              </li>
            </ol>
          </div>
        </div>
          ...
          <div className="bg-white border border-gray-200 rounded-xl p-4 flex flex-wrap gap-4 items-center">
            ...
            <label className="flex items-center gap-2 text-sm">
              ...
              <span className="text-gray-700">
                Deactivate old flower products not in this import
              </span>
            </label>
          </div>
                    ...
                    >
                      ...
                      <td className="px-3 py-2.5">
                        <span
                          className={`text-xs px-2 py-0.5 rounded-full font-medium ${
                            item.category.includes("Indica")
                              ? "bg-purple-100 text-purple-700"
                              : item.category.includes("Sativa")
                                ? "bg-yellow-100 text-yellow-700"
                                : item.category.includes("Hybrid")
                                  ? "bg-green-100 text-green-700"
                                  : item.category.includes("Ounce")
                      ...
                      <td className="px-3 py-2.5 text-center">
                        <input
                          ...
                          value={item.stock}
                          onChange={e =>
                            updateStock(idx, parseInt(e.target.value) || 0)
                          }
            ...
            <div className="bg-amber-50 border border-amber-200 rounded-xl p-4 flex items-start gap-3">
              ...
              <div>
                ...
                <p className="text-sm text-amber-700 mt-1">
                  Existing flower products NOT in this import will be
                  deactivated (hidden from the store). They won't be deleted and
                  can be reactivated manually from Admin &rarr; Products.
                </p>
              </div>
            </div>
          )}
      ...
      {step === "importing" && (
        <div className="bg-white border border-gray-200 rounded-2xl p-12 text-center space-y-4">
          ...
          <p className="text-sm text-gray-500">
            Creating product entries for each weight tier. This may take a
            moment.
          </p>
        </div>
      )}
