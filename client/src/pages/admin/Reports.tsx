...
const CanadaSalesDashboard = lazy(
  () => import("@/components/CanadaSalesDashboard")
);
  ...
  return (
    <div className="bg-white rounded-xl p-5 shadow-sm border border-gray-100">
      ...
      <div className="flex items-end justify-between">
          ...
          <Sparkline
            ...
            color={
              color.includes("green")
                ? "#16a34a"
                : color.includes("orange") || color.includes("F15929")
                  ? "#F15929"
                  : "#4B2D8E"
            }
          />
        ...
        {trend !== undefined && (
          <span
            className={`text-xs font-medium flex items-center gap-0.5 ${trend >= 0 ? "text-green-600" : "text-red-500"}`}
          >
...
export default function AdminReports() {
  ...
  const { data: orderStats, isLoading: orderStatsLoading } =
    trpc.admin.orderStats.useQuery({ days }, { refetchOnWindowFocus: true });
  const { data: topProducts, isLoading: topLoading } =
    trpc.admin.topProducts.useQuery(
      { limit: 10 },
      { refetchOnWindowFocus: true }
    );
  const { data: provinceSales, isLoading: provinceSalesLoading } =
    trpc.admin.geoAnalytics.salesByProvince.useQuery(
      { days },
      { refetchOnWindowFocus: true }
    );
  const aiInsightMutation = trpc.admin.geoAnalytics.aiMapInsight.useMutation();
  const [mapInsight, setMapInsight] = useState<string | null>(null);

  const handleRequestInsight = () => {
    if (!provinceSales || Object.keys(provinceSales).length === 0) return;
    aiInsightMutation.mutate(
      { salesData: provinceSales },
      { onSuccess: res => setMapInsight(res.insight) }
    );
  };

  const isLoading =
    statsLoading || orderStatsLoading || topLoading || provinceSalesLoading;

  // Revenue trend data (last N days, from orderStats)
  const revenueTrend = (orderStats ?? []).map((d: any) =>
    parseFloat(d.revenue)
  );
  ...
  const maxSold = topProducts?.length
    ? Math.max(...topProducts.map((p: any) => p.totalSold), 1)
    : 1;
  ...
  return (
    <div className="p-4 md:p-6 space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div className="bg-white rounded-xl p-5 shadow-sm border border-gray-100">
          ...
          <p className="text-3xl font-bold text-green-700">
            $
            {(stats?.totalRevenue ?? 0).toLocaleString("en-CA", {
        ...
        <div className="bg-white rounded-xl p-5 shadow-sm border border-gray-100">
          ...
          <p className="text-xs text-gray-400 mt-1">
            Over {stats?.totalOrders ?? 0} orders
          </p>
        </div>
        <div className="bg-white rounded-xl p-5 shadow-sm border border-gray-100">
          <p className="text-xs font-medium text-gray-500 uppercase tracking-wider mb-1">
          ...
          <p className="text-3xl font-bold text-blue-700">
            $
            {totalPeriodRevenue.toLocaleString("en-CA", {
              minimumFractionDigits: 2,
            })}
          </p>
          <p className="text-xs text-gray-400 mt-1">
            {totalPeriodOrders} orders in last {days} days
          </p>
        </div>
      </div>
          ...
          {Object.keys(statusConfig).map(status => {
            ...
            return (
              <div key={status} className="flex items-center gap-3 mb-3">
                <span
                  className={`inline-block px-2 py-0.5 rounded-full text-xs font-medium w-24 text-center shrink-0 ${cfg.color}`}
                >
                  {cfg.label}
                </span>
              ...
              <div className="mb-4 flex items-end gap-1 h-16">
                {dailyData.map((d: any, i: number) => {
                  const maxRev = Math.max(
                    ...dailyData.map((x: any) => parseFloat(x.revenue)),
                    1
                  );
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-5">
        <h2 className="font-semibold text-gray-800 mb-4 flex items-center gap-2">
          <Star size={16} className="text-[#F15929]" /> Top Products by Units
          Sold
        </h2>
        {topLoading ? (
            ...
            {["pending", "received", "confirmed", "refunded"].map(ps => {
              ...
              return (
                <div key={ps} className="flex items-center justify-between">
                  <span
                    className={`px-2.5 py-0.5 rounded-full text-xs font-medium ${payColors[ps] || "bg-gray-100 text-gray-600"}`}
                  >
      ...
      >
        <CanadaSalesDashboard
          data={provinceSales ?? {}}
          loading={provinceSalesLoading}
          onRequestInsight={handleRequestInsight}
          insight={mapInsight}
          insightLoading={aiInsightMutation.isPending}
        />
      </Suspense>
      ...
      <div className="bg-gray-50 rounded-xl border border-dashed border-gray-200 p-6 text-center">
        ...
        <p className="text-sm font-medium text-gray-500">
          More analytics coming as data grows
        </p>
        <p className="text-xs text-gray-400 mt-1">
          Customer lifetime value, repeat order rate, and coupon usage will
          appear here.
        </p>
      </div>
    </div>
