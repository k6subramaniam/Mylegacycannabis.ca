...
function Badge({
  ...
  return (
    <span
      className={`text-[10px] font-semibold uppercase px-2 py-0.5 rounded-full ${cls[color] || cls.gray}`}
    >
      {children}
    </span>
  );
...
function EditReviewModal({
  ...
  const { data: review, isLoading } = trpc.admin.reviews.get.useQuery({
    id: reviewId,
  });
      ...
      <div className="bg-white rounded-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
        ...
        <div className="p-6 space-y-5">
          ...
          <div className="bg-gray-50 rounded-xl p-4 text-sm space-y-1">
            <p>
              <span className="text-gray-500">User ID:</span> {review.userId}
            </p>
            <p>
      ...
      <div className="flex items-center justify-between mb-8">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">
            {total} total review{total !== 1 ? "s" : ""} — manage customer
            feedback
          </p>
        </div>
      </div>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-gray-200 bg-gray-50">
                  ...
                  <th className="text-right py-3 px-4 font-medium text-gray-500 text-xs uppercase">
                    Actions
                  </th>
                </tr>
              </thead>
                        ...
                        {r.tags &&
                          (r.tags as string[])
                            ...
                            .map((tag: string) => (
                              <span
                                key={tag}
                                className="text-[9px] px-1.5 py-0.5 rounded-full bg-purple-50 text-purple-600"
                              >
                                {TAG_LABELS[tag] || tag}
                              </span>
                            ))}
