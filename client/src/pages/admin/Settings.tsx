...
export default function AdminSettings() {
  ...
  const [idVerificationMode, setIdVerificationMode] = useState<"manual" | "ai">(
    "manual"
  );
  ...
  useEffect(() => {
    Promise.all([
      fetch("/api/auth/google-available")
        ...
        .catch(() => {}),
      fetch("/api/auth/sms-available")
        .then(r => r.json())
        .then(d => setSmsAvailable(d.available))
        .catch(() => {}),
      fetch("/api/auth/smtp-available")
        .then(r => r.json())
        .then(d => {
          setSmtpAvailable(d.available);
          setEmailProvider(d.provider || "none");
          setSmtpAdminEmail(d.adminEmail);
          setSmtpMissing(d.missing || []);
        })
        .catch(() => {}),
  ...
  useEffect(() => {
    if (settings) {
      ...
      setIdVerificationMode(
        (settings.id_verification_mode === "ai" ? "ai" : "manual") as
          | "manual"
          | "ai"
      );
      ...
      if (settings.store_hours) {
        try {
          setStoreHours(JSON.parse(settings.store_hours));
        } catch {}
      }
      if (settings.store_hours_note !== undefined)
        setHoursNote(settings.store_hours_note);
  ...
  const handleSaveHours = async () => {
    try {
      await Promise.all([
        updateSetting.mutateAsync({
          key: "store_hours_enabled",
          value: hoursEnabled ? "true" : "false",
        }),
        updateSetting.mutateAsync({
          key: "store_hours",
  ...
  const handleSaveBannerMessages = async () => {
    try {
      ...
      setBannerHasChanges(false);
      toast.success(
        `Banner updated with ${filtered.length} message${filtered.length !== 1 ? "s" : ""}.`
      );
    } catch (err: any) {
        ...
        <div className="px-6 py-4 border-b border-gray-100 flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-gray-500">
              Edit the promotional messages that scroll across the top of every
              page.
            </p>
          </div>
        </div>
                  ...
                  >
                    {bannerMessages
                      ...
                      .map((msg, i) => (
                        <span
                          key={`${setIndex}-${i}`}
                          className="flex items-center whitespace-nowrap text-xs md:text-sm font-medium mx-8"
                        >
                          {msg}
                        </span>
                      ))}
            ...
            {bannerMessages.map((msg, index) => (
              <div key={index} className="flex items-center gap-2 group">
                ...
                <button
                  ...
                  onClick={() => {
                    setBannerMessages(
                      bannerMessages.filter((_, i) => i !== index)
                    );
                    setBannerHasChanges(true);
                  }}
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
        <div className="px-6 py-4 border-b border-gray-100 flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-gray-500">
              Take the storefront offline with a customizable maintenance page.
            </p>
          </div>
        </div>
        ...
        <div className="p-6 space-y-6">
              ...
              <div className="flex items-start gap-3">
                ...
                <div>
                  <p className="text-sm font-medium text-red-800">
                    Store is currently offline for visitors
                  </p>
                  <p className="text-sm text-red-700 mt-1">
                    All storefront pages will display the maintenance message
                    below. The admin panel (/admin/*) remains fully accessible.
                  </p>
                </div>
              </div>
            </div>
          ...
          <div>
            ...
            <p className="text-xs text-gray-400 mt-1">
              This message is displayed on the maintenance page to all visitors.
            </p>
          </div>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
        <div className="px-6 py-4 border-b border-gray-100 flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-gray-500">
              Set your store's business hours displayed on the storefront.
            </p>
          </div>
        </div>
        ...
        <div className="p-6 space-y-6">
          ...
          <div className="space-y-3">
            ...
            <div className="border border-gray-200 rounded-xl overflow-hidden">
              <div className="grid grid-cols-[120px_1fr_auto_1fr_auto] md:grid-cols-[140px_1fr_auto_1fr_auto] items-center gap-x-3 gap-y-0 text-sm">
                {/* Table header */}
                <div className="bg-gray-50 px-4 py-2.5 font-medium text-xs text-gray-500 uppercase">
                  Day
                </div>
                <div className="bg-gray-50 px-2 py-2.5 font-medium text-xs text-gray-500 uppercase">
                  Opens
                </div>
                <div className="bg-gray-50 py-2.5 text-xs text-gray-400 text-center">
                  to
                </div>
                <div className="bg-gray-50 px-2 py-2.5 font-medium text-xs text-gray-500 uppercase">
                  Closes
                </div>
                <div className="bg-gray-50 px-3 py-2.5 font-medium text-xs text-gray-500 uppercase text-center">
                  Closed
                </div>
                ...
                {DAYS.map(day => {
                  ...
                  return (
                    <div key={day} className="contents">
                      ...
                      <div className="border-t border-gray-100 text-center text-gray-400 text-xs">
                        to
                      </div>
                      <div className="px-2 py-2 border-t border-gray-100">
                        <input
                      ...
                      <div className="px-3 py-2 border-t border-gray-100 flex justify-center">
                        <button
                          onClick={() =>
                            updateDayHours(day, "closed", !dayData.closed)
                          }
          ...
          <div>
            ...
            />
            <p className="text-xs text-gray-400 mt-1">
              Displayed below the hours schedule on the storefront.
            </p>
          </div>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
        <div className="px-6 py-4 border-b border-gray-100 flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-gray-500">
              Control whether customers must verify their identity before
              placing orders.
            </p>
          </div>
        </div>
        ...
        <div className="p-6 space-y-6">
          ...
          <div className="flex items-center justify-between">
            ...
            <button
              onClick={() => {
                setIdVerificationEnabled(!idVerificationEnabled);
                setIdHasChanges(true);
              }}
              className={`relative inline-flex h-7 w-14 items-center rounded-full transition-colors focus:outline-none focus:ring-2 focus:ring-[#4B2D8E] focus:ring-offset-2 ${
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                ...
                >
                  ...
                  <p className="text-xs text-gray-500 leading-relaxed">
                    All submitted IDs are queued for admin review. You approve
                    or reject each submission manually.
                  </p>
                </button>
                  ...
                  <div className="flex items-center gap-3 mb-2">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-gray-500">
                        AI reviews IDs instantly
                      </p>
                    </div>
                  </div>
                  <p className="text-xs text-gray-500 leading-relaxed">
                    AI automatically verifies IDs using vision. High-confidence
                    approvals are instant; low-confidence submissions are
                ...
                <div className="mt-3 rounded-lg p-3 bg-violet-50 border border-violet-200">
                  <div className="flex items-start gap-2">
                    ...
                    <p className="text-xs text-violet-700">
                      AI verification uses the configured AI provider (Settings
                      &rarr; AI Configuration). Ensure an API key is set. The AI
                      checks for valid Canadian government-issued photo ID and
                      verifies the holder is 19+. If confidence is low, the
                      submission is flagged for your manual review.
                    </p>
                  </div>
                </div>
              )}
          ...
          >
            <div className="flex items-start gap-3">
              ...
              <div>
                <p
                  className={`text-sm font-medium ${idVerificationEnabled ? "text-blue-800" : "text-amber-800"}`}
                >
                  {idVerificationEnabled ? "When Enabled:" : "When Disabled:"}
                </p>
                <ul
                  className={`text-sm mt-1.5 space-y-1 ${idVerificationEnabled ? "text-blue-700" : "text-amber-700"}`}
                >
                  {idVerificationEnabled ? (
                    <>
                      <li className="flex items-start gap-2">
                        ...
                        <span>
                          Registered users must verify their ID once before
                          placing orders
                        </span>
                      </li>
                      <li className="flex items-start gap-2">
                        <CheckCircle size={14} className="shrink-0 mt-0.5" />
                        <span>
                          Guest customers must submit ID at every checkout
                        </span>
                      </li>
                      <li className="flex items-start gap-2">
                        <CheckCircle size={14} className="shrink-0 mt-0.5" />
                        <span>
                          ID verification email notifications are sent to
                      ...
                      <li className="flex items-start gap-2">
                        <CheckCircle size={14} className="shrink-0 mt-0.5" />
                        <span>
                          Pending verification alerts appear on the admin
                          dashboard
                        </span>
                      </li>
                    </>
                    ...
                    <>
                      <li className="flex items-start gap-2">
                        ...
                        <span>
                          All customers (registered and guests) can place orders
                          without ID checks
                        </span>
                      </li>
                      <li className="flex items-start gap-2">
                        <ShieldOff size={14} className="shrink-0 mt-0.5" />
                        <span>
                          ID verification UI is hidden from checkout, cart, and
                          account pages
                        </span>
                      </li>
                      <li className="flex items-start gap-2">
                        <ShieldOff size={14} className="shrink-0 mt-0.5" />
                        <span>
                          ID-related email notifications are suppressed
                        </span>
                      </li>
                      <li className="flex items-start gap-2">
                        <ShieldOff size={14} className="shrink-0 mt-0.5" />
                        <span>
                          Existing verified users retain their verified status
                        </span>
                      </li>
                    </>
                  )}
          ...
          {idHasChanges && (
            <div className="flex items-center gap-3 pt-2">
              ...
              <button
                onClick={() => {
                  if (settings) {
                    setIdVerificationEnabled(
                      settings.id_verification_enabled !== "false"
                    );
                  }
                  setIdHasChanges(false);
                }}
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
        <div className="px-6 py-4 border-b border-gray-100 flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-gray-500">
              Google social login, Twilio SMS, and SMTP email configuration
              status.
            </p>
          </div>
        </div>
                        ...
                        <ol className="text-xs text-gray-500 space-y-1 list-decimal list-inside">
                          <li>
                            ...
                            <a
                              ...
                              className="text-[#4B2D8E] underline"
                            >
                          ...
                          <li>
                            ...
                            <code className="bg-gray-100 px-1.5 py-0.5 rounded text-[#4B2D8E]">
                              https://yourdomain.com/api/auth/google/callback
                            </code>
                          </li>
                          <li>
                            Set these environment variables on your server (e.g.
                            Railway):
                            <div className="mt-1 space-y-0.5">
                              <code className="block bg-gray-100 px-2 py-1 rounded text-[10px]">
                          ...
                          <li>
                            Restart the server — the button will activate
                            automatically
                          </li>
                        </ol>
                      </div>
                ...
                <div className="flex items-start gap-4">
                  ...
                  >
                    <Smartphone
                      size={24}
                      className={
                        smsAvailable ? "text-green-600" : "text-amber-600"
                      }
                    />
                  </div>
                  <div className="flex-1 min-w-0">
                    ...
                    {!smsAvailable && (
                      <div className="mt-3 bg-white rounded-lg border border-amber-200 p-3">
                        ...
                        <ol className="text-xs text-gray-500 space-y-1 list-decimal list-inside">
                          ...
                          <li>
                            Restart the server — SMS verification will activate
                            automatically
                          </li>
                        </ol>
                        <div className="mt-2 flex items-start gap-1.5">
                          ...
                          <p className="text-[10px] text-amber-600">
                            While SMS is not configured, phone OTP codes are
                            logged to the server console for testing.
                          </p>
                        </div>
                      </div>
                    )}
                ...
                <div className="flex items-start gap-4">
                  ...
                  >
                    <MessageSquare
                      size={24}
                      className={
                        smtpAvailable ? "text-green-600" : "text-amber-600"
                      }
                    />
                  </div>
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2">
                      ...
                      >
                        {smtpAvailable
                          ? `Active (${emailProvider === "resend" ? "Resend" : "SMTP"})`
                          : "Not Configured"}
                      </span>
                    </div>
                    <p className="text-sm text-gray-500 mt-1">
                    ...
                    {!smtpAvailable && (
                      <div className="mt-3 bg-white rounded-lg border border-amber-200 p-3">
                        ...
                        <p className="text-xs font-semibold text-gray-500 mb-1.5">
                          Option B — Gmail SMTP (requires Railway Pro plan):
                        </p>
                        <ol className="text-xs text-gray-400 space-y-1 list-decimal list-inside">
                          ...
                          <li>
                            ...
                            <a
                              ...
                              target="_blank"
                              rel="noopener noreferrer"
                              className="text-[#4B2D8E]/60 underline"
                            >
                          ...
                          <li>
                            Note: Railway Hobby plan blocks SMTP ports 465/587.
                        ...
                        <div className="mt-2 flex items-start gap-1.5">
                          ...
                          />
                          <p className="text-[10px] text-amber-600">
                            While email is not configured, OTP codes are logged
                            to the server console for testing.
                          </p>
                        </div>
                      </div>
                    )}
                    ...
                    <ul className="mt-1.5 space-y-1">
                      <li className="flex items-center gap-2">
                        ...
                        <span>
                          Email Delivery —{" "}
                          {smtpAvailable
                            ? "active via SMTP (real emails sent)"
                            : "not configured (OTP codes logged to console)"}
                        </span>
                      </li>
                      <li className="flex items-center gap-2">
                        <CheckCircle size={14} className="text-green-500" />
...
function AiConfigSection() {
  ...
  useEffect(() => {
    if (aiConfig) {
      ...
      setFallbackProvider(
        (aiConfig.fallbackProvider || "") as "openai" | "gemini" | ""
      );
  ...
  const GEMINI_MODELS = [
    ...
    { value: "gemini-3.1-flash-lite-preview", label: "Gemini 3.1 Flash Lite" },
    {
      ...
      label: "Gemini 3.1 Pro — newest flagship",
    },
  ];

  const getModelsForProvider = (p: string) =>
    p === "gemini" ? GEMINI_MODELS : OPENAI_MODELS;
  const models = getModelsForProvider(provider);
  const fallbackModels = fallbackProvider
    ? getModelsForProvider(fallbackProvider)
    : [];
    ...
    <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
      <div className="px-6 py-4 border-b border-gray-100 flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-sm text-gray-500">
            Configure primary and fallback AI providers. If the primary fails,
          ...
          <>
                ...
                {aiConfig?.apiKeySet && !apiKeyInput && (
                  <div className="flex items-center gap-2 mb-1.5 px-2.5 py-1.5 bg-green-50 border border-green-200 rounded-lg">
                    ...
                    <span className="text-xs text-green-700">
                      Key set:{" "}
                      <code className="bg-green-100 px-1 py-0.5 rounded text-[10px]">
            ...
            <div className="bg-gradient-to-r from-amber-50 to-orange-50 rounded-xl p-5 border border-amber-100">
              ...
              <p className="text-xs text-amber-700 mb-4">
                If the primary provider fails or is unreachable, this provider
              ...
              {fallbackProvider && (
                <>
                  ...
                  <div className="mb-3">
                    <label className="flex items-center gap-2 text-xs font-medium text-gray-600 mb-1.5">
                      <Key size={12} />{" "}
                      {fallbackProvider === "openai" ? "OpenAI" : "Gemini"}{" "}
                      Fallback API Key
                    </label>
                    {aiConfig?.fallbackApiKeySet && !fallbackApiKeyInput && (
                      <div className="flex items-center gap-2 mb-1.5 px-2.5 py-1.5 bg-green-50 border border-green-200 rounded-lg">
                        ...
                        <span className="text-xs text-green-700">
                          Key set:{" "}
                          <code className="bg-green-100 px-1 py-0.5 rounded text-[10px]">
                            {aiConfig?.fallbackApiKeyPreview}
                          </code>
                        </span>
                      </div>
                    )}
                    <div className="relative">
                      <input
                        ...
                        onChange={e => {
                          setFallbackApiKeyInput(e.target.value);
                          setHasChanges(true);
                        }}
                        placeholder={
                          fallbackProvider === "gemini"
                ...
                <div className="text-sm text-gray-600">
                  ...
                  <ul className="mt-1.5 space-y-1 text-xs">
                    <li className="flex items-center gap-2">
                      ...
                      Email template generation (AI Generate)
                    </li>
                    <li className="flex items-center gap-2">
                      <CheckCircle size={12} className="text-violet-500" />{" "}
                      Email template improvement (AI Improve)
                    </li>
                    <li className="flex items-center gap-2">
                      <CheckCircle size={12} className="text-violet-500" /> Menu
                      image import (AI Vision)
                    </li>
                    <li className="flex items-center gap-2">
                      <CheckCircle size={12} className="text-violet-500" />{" "}
                    ...
                    <li className="flex items-center gap-2">
                      <CheckCircle size={12} className="text-violet-500" /> AI
                      Chat assistant
                    </li>
                  </ul>
                </div>
                  ...
                  <div>
                    ...
                    {testResult.success ? (
                      <p className="text-xs text-green-700 mt-1">
                        ...
                        &mdash; Latency: {testResult.latency}ms &mdash; Reply: "
                        {testResult.reply}"
                      </p>
                    ) : (
...
function LogoManagementSection() {
  ...
  const uploadMutation = trpc.admin.emailLogo.upload.useMutation({
    onSuccess: data => {
      ...
      utils.store.siteConfig.invalidate();
      toast.success(
        "Logo uploaded and applied globally! All pages, emails, and the admin panel will now use the new logo."
      );
    },
  ...
  const handleReset = () => {
    if (
      window.confirm(
        "Reset the logo to the default /logo.png? This will affect all pages and emails."
      )
    ) {
    ...
    <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
      <div className="px-6 py-4 border-b border-gray-100 flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-sm text-gray-500">
            Upload a logo that updates globally across the website, admin panel,
            ...
            <div>
              ...
              {!isDefault && (
                <p className="text-xs text-gray-400 mt-2 truncate">
                  URL:{" "}
                  <code className="bg-gray-50 px-1.5 py-0.5 rounded text-[10px]">
                    {cleanUrl}
                  </code>
                </p>
              )}
                  ...
                  <div className="absolute left-1/2 -translate-x-1/2 bottom-full mb-2 w-72 bg-gray-900 text-white text-xs rounded-lg px-4 py-3 shadow-xl opacity-0 pointer-events-none group-hover/tip:opacity-100 group-hover/tip:pointer-events-auto transition-opacity z-50">
                    ...
                    <ul className="space-y-1 text-gray-300">
                      <li>
                        ...
                        logo.png &mdash; 1024 x 572 px (RGBA)
                      </li>
                      <li>
                        <span className="text-white font-medium">
                          Aspect ratio:
                      ...
                      <li>
                        ...
                        h-10 (40 px) mobile, h-14 (56 px) desktop
                      </li>
                      <li>
                        <span className="text-white font-medium">Width:</span>{" "}
                        auto-scales to maintain ratio
                      </li>
                      <li>
                        <span className="text-white font-medium">
                          Best format:
              ...
              >
                ...
                {uploadMutation.isPending ? (
                  <div className="flex flex-col items-center gap-2">
                    <Loader2
                      size={28}
                      className="animate-spin text-[#4B2D8E]"
                    />
                  ...
                  <div className="flex flex-col items-center gap-2">
                    ...
                    <div className="text-center">
                      ...
                      <p className="text-xs text-gray-400 mt-0.5">
                        PNG, JPG, SVG, or WebP (max 5 MB). Transparent PNG
                        recommended.
                      </p>
                    </div>
                  </div>
            ...
            <div className="bg-[#4B2D8E]/5 rounded-xl p-4">
              <div className="flex items-start gap-2">
                ...
                <div className="text-sm text-gray-600">
                  ...
                  <ul className="mt-1.5 space-y-1 text-xs">
                    <li className="flex items-center gap-2">
                      <CheckCircle size={12} className="text-[#4B2D8E]" />{" "}
                      Website header and footer
                    </li>
                    <li className="flex items-center gap-2">
                      <CheckCircle size={12} className="text-[#4B2D8E]" /> Age
                      gate overlay
                    </li>
                    <li className="flex items-center gap-2">
                      <CheckCircle size={12} className="text-[#4B2D8E]" />{" "}
                    ...
                    <li className="flex items-center gap-2">
                      <CheckCircle size={12} className="text-[#4B2D8E]" /> Admin
                      panel sidebar and mobile header
                    </li>
                    <li className="flex items-center gap-2">
                      <CheckCircle size={12} className="text-[#4B2D8E]" />{" "}
                      Login, Register, and Complete Profile pages
                    </li>
                    <li className="flex items-center gap-2">
                      <CheckCircle size={12} className="text-[#4B2D8E]" /> All
                      email template headers and footers
                    </li>
                    <li className="flex items-center gap-2">
                      <CheckCircle size={12} className="text-[#4B2D8E]" /> OTP
                      verification and admin notification emails
                    </li>
                    <li className="flex items-center gap-2">
                      <CheckCircle size={12} className="text-[#4B2D8E]" /> SEO
...
function PushBroadcastSection() {
  const {
    ...
    isLoading,
    refetch,
  } = trpc.store.pushStats.useQuery(undefined, {
    staleTime: 30_000,
    retry: false,
  });
  const broadcastMut = trpc.store.pushBroadcast.useMutation({
    onSuccess: data => {
      toast.success(
        `Push sent to ${data.sent} subscriber(s), ${data.failed} failed`
      );
      setTitle("");
      setBody("");
      setUrl("");
    ...
    <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
      <div className="px-6 py-4 border-b border-gray-100 flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-sm text-gray-500">
            Send PWA push notifications to all subscribed devices.
          </p>
        </div>
      </div>
        ...
        {isLoading ? (
          ...
          <div className="grid grid-cols-3 gap-4">
            <div className="bg-purple-50 rounded-xl p-4 text-center">
              <p className="text-2xl font-bold text-[#4B2D8E]">
                {stats.active ?? 0}
              </p>
        ...
        {stats && (stats.active ?? 0) > 0 && (
          <div className="border-t border-gray-100 pt-6 space-y-4">
            ...
            >
              ...
              Send to {stats.active ?? 0} subscriber
              {(stats.active ?? 0) !== 1 ? "s" : ""}
            </button>
          </div>
        )}
          ...
          <div className="border-t border-gray-100 pt-6">
            ...
            <div className="space-y-2 max-h-60 overflow-y-auto">
              {stats.recentLogs.map((log: any, i: number) => (
                <div
                  key={log.id || i}
                  className="flex items-center gap-3 text-xs p-2 rounded bg-gray-50"
                >
                  <span
