...
function CustomerModal({
  ...
  const { data: user, isLoading } = trpc.admin.users.get.useQuery({
    id: userId,
  });
      ...
      <div className="bg-white rounded-2xl w-full max-w-2xl my-8 shadow-2xl">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100">
          <div className="flex items-center gap-3">
            <div
              className={`w-10 h-10 rounded-full flex items-center justify-center text-white shrink-0 ${isLocked ? "bg-red-500" : "bg-[#4B2D8E]"}`}
            >
          ...
          <div className="p-6">
              ...
              <div className="space-y-5">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  ...
                  <div className="flex items-center gap-3 pt-2">
                    ...
                    >
                      <span
                        className={`inline-block h-4 w-4 transform rounded-full bg-white shadow transition-transform ${editIdVerified ? "translate-x-6" : "translate-x-1"}`}
                      />
                    </button>
                    <span
                      className={`text-xs ${editIdVerified ? "text-green-600" : "text-gray-400"}`}
                    >
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 pt-1">
                  {[
                    ...
                    {
                      label: "Last Active",
                      value: fmtDate(user.lastSignedIn),
                      icon: <Clock size={12} />,
                    },
                    {
                      label: "Reward Points",
                      value: String(user.rewardPoints ?? 0),
                      icon: <Star size={12} />,
                    },
                    {
                      label: "Login Method",
                      value: (user as any).loginMethod || "email",
                      icon: <Mail size={12} />,
                    },
                  ].map(item => (
                ...
                {((user as any).lastIp || (user as any).registrationIp) && (
                  <div className="bg-blue-50/50 border border-blue-100 rounded-xl p-4 space-y-2">
                    ...
                    <div className="grid grid-cols-2 gap-3 text-xs">
                      ...
                      {((user as any).lastGeoCity ||
                        (user as any).lastGeoRegion ||
                        (user as any).lastGeoCountry) && (
            ...
            {tab === "orders" && (
              <div className="space-y-3">
                <p className="text-xs text-gray-500">
                  {orders?.length ?? 0} orders matched to this customer
                </p>
                {!orders?.length ? (
              ...
              <div className="space-y-5">
                ...
                <div className="space-y-3">
                  ...
                  <div className="flex gap-2">
                    <button
                      onClick={() =>
                        setPointsDelta(v => String(Number(v || 0) - 10))
                      }
                  ...
                  {pointsDelta && Number(pointsDelta) !== 0 && (
                    <p className="text-xs text-gray-500">
                      ...
                      <strong>
                        {Math.max(
                          0,
                          (user.rewardPoints ?? 0) + Number(pointsDelta)
                        )}
                      </strong>{" "}
                      points
                    </p>
            ...
            {tab === "security" && (
              <div className="space-y-6">
                {/* Lock / Unlock */}
                <div
                  className={`p-4 rounded-xl border-2 ${isLocked ? "border-red-200 bg-red-50" : "border-gray-200"}`}
                >
                  <div className="flex items-center justify-between mb-3">
                    <div>
                      ...
                      <p className="text-xs text-gray-500 mt-0.5">
                        {isLocked
                          ? "This account is currently locked. The customer cannot log in."
                          : "Account is active. Customer can log in normally."}
                      </p>
                    </div>
                    <span
                      className={`px-2.5 py-1 rounded-full text-xs font-medium ${isLocked ? "bg-red-100 text-red-600" : "bg-green-100 text-green-700"}`}
                    >
                ...
                <div className="p-4 rounded-xl border border-gray-200 space-y-3">
                  <div>
                    ...
                    <p className="text-xs text-gray-500 mt-0.5">
                      Sets a new password for the customer's account. They can
                      also log in via OTP at any time.
                    </p>
                  </div>
                  <input
                    type="password"
                  ...
                  <button
                    ...
                    onClick={() =>
                      resetPwMut.mutate({ id: userId, newPassword })
                    }
                ...
                <div className="p-4 rounded-xl border-2 border-red-200 bg-red-50 space-y-3">
                  <div>
                    <p className="font-medium text-red-700 text-sm flex items-center gap-2">
                      <Trash2 size={14} /> Delete Account
                    </p>
                    <p className="text-xs text-red-500 mt-0.5">
                      Permanently removes this customer account. This cannot be
                      undone.
                    </p>
                  </div>
                  {!showDeleteConfirm ? (
                    ...
                    <div className="space-y-2">
                      ...
                      <div className="flex gap-2">
                        <button
                          disabled={
                            deleteTyped !== "DELETE" || deleteMut.isPending
                          }
                          onClick={() =>
                            deleteMut.mutate({ id: userId, confirm: true })
                          }
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <p className="text-xs text-gray-400 mb-3">
                    Internal notes visible only to admins. Automatically
                    appended with lock/unlock and password reset events.
                  </p>
                  <textarea
                    value={adminNotes}
                    onChange={e => {
...
function AddUserModal({
  ...
  const createMut = trpc.admin.users.create.useMutation({
    onSuccess: () => {
      toast.success(
        `${role === "admin" ? "Admin" : "Customer"} created! Welcome email sent.`
      );
      onSuccess();
      onClose();
    },
      ...
      >
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100">
          <h2 className="font-semibold text-gray-800 flex items-center gap-2">
            <UserPlus size={18} className="text-[#4B2D8E]" /> Add User
          </h2>
          <button
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          ...
          <div>
            ...
            <div className="flex gap-2">
              <span className="px-3 py-2.5 rounded-xl border border-gray-200 bg-gray-50 text-sm text-gray-500">
                +1
              </span>
              <input
                type="tel"
          ...
          >
            ...
            Create {role === "admin" ? "Admin" : "Customer"} & Send Welcome
            Email
          </button>
        </form>
      </div>
  ...
  return (
    <div className="p-4 md:p-6 space-y-6">
      ...
      <form onSubmit={handleSearch} className="flex gap-2 max-w-md">
        ...
        <button
          type="submit"
          className="px-4 py-2.5 bg-[#4B2D8E] text-white rounded-xl text-sm font-medium hover:bg-[#3a2270] transition-colors"
        >
          Search
        </button>
        {search && (
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-gray-100 bg-gray-50/50">
                ...
                <th className="text-right px-4 py-3 text-xs font-medium text-gray-500 uppercase hidden md:table-cell">
                  Joined
                </th>
                ...
                data?.data.map((user: any) => (
                  ...
                  >
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-3">
                        <div
                          className={`w-9 h-9 rounded-full flex items-center justify-center shrink-0 ${user.isLocked ? "bg-red-100" : "bg-[#4B2D8E]/10"}`}
                        >
                          {user.isLocked ? (
                            <Lock size={14} className="text-red-500" />
                          ) : (
                    ...
                    <td className="px-4 py-3 text-center">
                      {user.role === "admin" ? (
                        <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-medium bg-[#4B2D8E]/10 text-[#4B2D8E]">
                          <Crown size={9} /> Admin
                        </span>
                      ) : (
                    ...
                    <td className="px-4 py-3 text-center text-gray-600 text-sm hidden lg:table-cell">
                      <span className="flex items-center justify-center gap-1">
                        <Star size={11} className="text-[#F15929]" />
                        {user.rewardPoints ?? 0}
                      </span>
                    </td>
