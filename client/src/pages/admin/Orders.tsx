...
function validateCanadaPostTracking(raw: string): TrackingValidation {
  ...
  if (/^\d{16}$/.test(v))
    return { valid: true, format: "Domestic PIN (16-digit)", error: null };

  // 13 alphanumeric — S10 international (2 letters + 9 digits + CA)
  if (/^[A-Z]{2}\d{9}CA$/.test(v))
    return {
      valid: true,
      format: "International / Xpresspost (13-char)",
      error: null,
    };

  // 12 digits — shorter domestic PIN
  if (/^\d{12}$/.test(v))
    return { valid: true, format: "Domestic PIN (12-digit)", error: null };

  // 11 alphanumeric — 2 letters + 7 digits + CA
  if (/^[A-Z]{2}\d{7}CA$/.test(v))
    return { valid: true, format: "Domestic (11-char)", error: null };
  ...
  if (/^\d+$/.test(v)) {
    if (v.length < 12)
      return {
        valid: false,
        format: null,
        error: `Too short — Canada Post PINs are 12 or 16 digits (entered ${v.length})`,
      };
    if (v.length > 16)
      return {
        valid: false,
        format: null,
        error: `Too long — Canada Post PINs are 12 or 16 digits (entered ${v.length})`,
      };
    return {
      valid: false,
      format: null,
      error: `Invalid length — Canada Post PINs are 12 or 16 digits (entered ${v.length})`,
    };
  }
  ...
  if (/^[A-Z]{2}.*CA$/.test(v)) {
    ...
    if (!/^\d+$/.test(mid))
      return {
        valid: false,
        format: null,
        error: "Middle characters must be digits (e.g. AB123456789CA)",
      };
    if (mid.length !== 9 && mid.length !== 7)
      return {
        valid: false,
        format: null,
        error: `Alphanumeric tracking should be 11 or 13 chars ending in CA (entered ${v.length})`,
      };
  }
  ...
  return {
    ...
    format: null,
    error:
      "Not a valid Canada Post tracking number. Expected: 16 digits, 12 digits, or 2 letters + digits + CA (11 or 13 chars).",
  };
}
...
function CanadaPostTrackingWidget({ pin }: { pin: string }) {
  const { data, isLoading, error } = trpc.store.trackShipment.useQuery(
    { pin },
    {
      ...
      <div className="flex items-center justify-between">
        ...
        {data.expectedDelivery && (
          <span className="text-[10px] text-gray-500">
            Est.{" "}
            {new Date(data.expectedDelivery).toLocaleDateString("en-CA", {
              month: "short",
              day: "numeric",
            })}
          </span>
        ...
        <div className="border-t border-gray-200 pt-2 mt-2 space-y-1.5">
          ...
          {data.events.slice(0, 3).map((evt: any, i: number) => (
            <div key={i} className="flex items-start gap-2 text-[10px]">
              <div
                className={`mt-1 w-1.5 h-1.5 rounded-full shrink-0 ${i === 0 ? "bg-[#4B2D8E]" : "bg-gray-300"}`}
              />
              <div>
                ...
                {evt.location && (
                  <span className="text-gray-400"> — {evt.location}</span>
                )}
...
function OrderDetail({ id }: { id: number }) {
  ...
  const updateStatus = trpc.admin.orders.updateStatus.useMutation({
    onSuccess: () => {
      utils.admin.orders.get.invalidate({ id });
      utils.admin.orders.list.invalidate();
      utils.admin.stats.invalidate();
      toast.success("Status updated");
    },
  });
  const updatePayment = trpc.admin.orders.updatePayment.useMutation({
    onSuccess: () => {
      utils.admin.orders.get.invalidate({ id });
      utils.admin.orders.list.invalidate();
      utils.admin.stats.invalidate();
  ...
  const addTracking = trpc.admin.orders.addTracking.useMutation({
    onSuccess: () => {
      utils.admin.orders.get.invalidate({ id });
      toast.success("Tracking added");
    },
  });
  const addNote = trpc.admin.orders.addNote.useMutation({
    onSuccess: () => {
      utils.admin.orders.get.invalidate({ id });
  ...
  const trackingValidation = useMemo(
    () => validateCanadaPostTracking(trackingNum),
    [trackingNum]
  );
  ...
  const trackingBlocked =
    paymentGated || !["confirmed", "processing"].includes(order.status);

  return (
    <div className="p-4 md:p-6 space-y-6">
      <div className="flex items-center gap-3">
        <Link href="/admin/orders" className="p-2 rounded-lg hover:bg-gray-100">
          <ArrowLeft size={18} />
        </Link>
        <div>
        ...
        <div className="bg-orange-50 border border-orange-200 rounded-xl p-4 flex items-start gap-3">
          ...
          <div>
            <p className="text-sm font-semibold text-orange-800">
              Held — Awaiting ID Verification
            </p>
            <p className="text-xs text-orange-700 mt-1">
              This order was placed by a guest whose ID is under review. Do not
              process or ship until identity is confirmed in{" "}
              <Link
          ...
          <div className="bg-red-50 border border-red-200 rounded-xl p-4 flex items-start gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-red-700 mt-1">
                No e-Transfer payment has been detected for this order. The
                order cannot advance until payment is received and confirmed.
                Check the{" "}
                <Link
                  href="/admin/payments"
                  className="underline font-semibold"
        ...
        <div className="bg-blue-50 border border-blue-200 rounded-xl p-4 flex items-start gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-blue-700 mt-1">
              A payment has been matched to this order but has not been verified
              yet. Confirm the payment amount in the{" "}
              <Link href="/admin/payments" className="underline font-semibold">
                Payments
              </Link>{" "}
              panel to unlock order processing.
            </p>
          </div>
        </div>
      )}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-6">
                  ...
                  {(order as any).shippingMethodName && (
                    <div>
                      ...
                      <p className="text-sm text-gray-800 font-medium flex items-center gap-1.5">
                        {(order as any).shippingMethod === "DOM.PC" ? (
                          <Timer size={14} className="text-[#F15929]" />
                        ) : (order as any).shippingMethod === "DOM.XP" ? (
                          <Zap size={14} className="text-[#4B2D8E]" />
                        ) : (order as any).shippingMethod === "DOM.EP" ? (
          ...
          <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-5">
            ...
            {order.adminNotes && (
              <pre className="text-xs text-gray-600 bg-gray-50 p-3 rounded-lg mb-3 whitespace-pre-wrap font-sans">
                {order.adminNotes}
              </pre>
            )}
            <div className="flex gap-2">
              ...
              />
              <button
                onClick={() => note && addNote.mutate({ id, note })}
                disabled={!note || addNote.isPending}
                className="bg-[#4B2D8E] text-white px-4 py-2 rounded-lg text-sm font-medium hover:bg-[#3a2270] disabled:opacity-50"
              >
                Add
              </button>
            </div>
          ...
          <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-5">
            ...
            <div className="space-y-3">
              ...
              <div className="flex items-center justify-between">
                ...
                <span
                  className={`px-2.5 py-1 rounded-full text-xs font-medium ${statusColors[order.paymentStatus]}`}
                >
                    ...
                    <div className="bg-gray-50 rounded-xl p-3 space-y-1.5 text-sm">
                      <div className="flex items-center gap-2 mb-2">
                        ...
                        <span
                          className={`ml-auto px-2 py-0.5 rounded-full text-[10px] font-medium ${
                            pr.matchConfidence === "exact"
                              ? "bg-green-100 text-green-700"
                              : pr.matchConfidence === "high"
                                ? "bg-blue-100 text-blue-700"
                                : pr.matchConfidence === "low"
                      ...
                      {pr.senderName && (
                        <div className="flex justify-between">
                          ...
                          <span className="font-medium">{pr.senderName}</span>
                        </div>
                      )}
                      {pr.senderEmail && (
                        <div className="flex justify-between">
                      ...
                      <div className="flex justify-between">
                        ...
                        <span className="text-xs">{pr.matchMethod}</span>
                      </div>
                      {pr.receivedAt && (
                        <div className="flex justify-between">
                          <span className="text-gray-500">Received</span>
                          <span className="text-xs">
                            {new Date(pr.receivedAt).toLocaleString("en-CA")}
                          </span>
                        </div>
                      )}
                      ...
                      {pr.senderName &&
                        order.guestName &&
                        pr.senderName.toLowerCase() !==
                          order.guestName.toLowerCase() && (
                          <div className="bg-orange-50 border border-orange-200 rounded-lg p-2 mt-2 flex items-start gap-1.5">
                            ...
                            <p className="text-[10px] text-orange-700">
                              Sender name does not match customer. Third-party
                              payment?
                            </p>
                          </div>
                        )}
                ...
                <div className="bg-yellow-50 border border-yellow-200 rounded-xl p-3 flex items-start gap-2">
                  ...
                  <div>
                    ...
                    <p className="text-[10px] text-yellow-700 mt-0.5">
                      No e-transfer has been linked yet. Check the{" "}
                      <Link
                        href="/admin/payments"
                        className="underline font-semibold"
                      >
              ...
              {order.paymentStatus === "received" && (
                <button
                  onClick={() =>
                    updatePayment.mutate({ id, paymentStatus: "confirmed" })
                  }
            ...
            {paymentGated && order.status === "pending" && (
              <div className="mb-3 bg-amber-50 border border-amber-200 rounded-lg p-2.5 flex items-start gap-2">
                ...
                <p className="text-xs text-amber-700">
                  Payment must be{" "}
                  <span className="font-semibold">Confirmed</span> before order
                  can advance past Pending.
                </p>
              </div>
            )}
            <select
              value={order.status}
              onChange={e =>
                updateStatus.mutate({ id, status: e.target.value as any })
              }
              disabled={
                paymentGated &&
              ...
              title={
                paymentGated
                  ? "Payment must be confirmed before advancing the order"
                  : undefined
              }
            ...
            {order.trackingNumber ? (
              ...
              <div className="space-y-2">
                      ...
                      if (!trackingValidation.valid) {
                        toast.error(
                          trackingValidation.error ||
                            "Invalid Canada Post tracking number"
                        );
                        return;
                      }
                ...
                <p className="text-[11px] text-gray-400">
                  Canada Post: 16 digits, 12 digits, or 2 letters + digits + CA
                  (11 or 13 chars)
                </p>
                <p className="text-[11px] text-blue-500 flex items-center gap-1 mt-1">
                  <Info size={11} /> Adding tracking will auto-set the order to{" "}
                  <span className="font-semibold">Shipped</span> and award
                  reward points.
                </p>
              </div>
            )}
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-gray-100 bg-gray-50/50">
                ...
                <th className="text-right px-4 py-3 text-xs font-medium text-gray-500 uppercase hidden lg:table-cell">
                  Date
                </th>
            ...
            <tbody>
              {isLoading ? (
                ...
                data?.data.map((order: any) => (
                  <tr
                    key={order.id}
                    className={`border-b border-gray-50 hover:bg-gray-50/50 ${isIdPending(order) ? "bg-yellow-50/40" : ""}`}
                  >
                    <td className="px-4 py-3 font-mono text-xs font-semibold text-[#4B2D8E]">
                      {order.orderNumber}
                    </td>
                    ...
                    <td className="px-4 py-3 hidden md:table-cell">
                      <span
                        className={`inline-block px-2.5 py-1 rounded-full text-xs font-medium ${statusColors[order.paymentStatus]}`}
                      >
                        {formatStatus(order.paymentStatus)}
                      </span>
                    </td>
                    ...
                    <td className="px-4 py-3 text-right">
                      <Link
                        ...
                        className="p-2 rounded-lg hover:bg-blue-50 text-[#4B2D8E] inline-block"
                      >
                        <Eye size={16} />
                      </Link>
                    </td>
