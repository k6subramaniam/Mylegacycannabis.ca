...
export function registerCustomAuthRoutes(app: Express) {
        ...
        if (type === "email") {
          ...
          if (smsResult.sent) {
            ...
            res.json({
              ...
              message:
                smsResult.reason ||
                "SMS service is not yet configured. The code has been logged for admin verification.",
              method: "sms_pending",
              fallback: true,
            });
          }
  ...
  app.post(
    ...
    async (req: Request, res: Response) => {
      try {
        const { identifier, code, type, purpose, registrationData } =
          req.body as {
            identifier: string;
            code: string;
            type: "email" | "sms";
        ...
        if (!identifier || !code || !type) {
          res
            .status(400)
            .json({ error: "identifier, code, and type are required" });
          return;
        }
        ...
        if (!result.valid) {
          res
            .status(400)
            .json({ error: result.reason || "Invalid verification code" });
          return;
        }
        ...
        if (purpose === "register") {
          ...
          const fullName =
            registrationData?.name || `${firstName} ${lastName}`.trim();

          if (!fullName || !registrationData?.phone) {
            res.status(400).json({
              error: "Name and phone number are required for registration",
            });
            return;
          }
          ...
          if (registrationData.email) {
            ...
            if (isDisposableEmail(registrationData.email)) {
              ...
              await db.logSystem({
                ...
                message: `Disposable email rejected: ${registrationData.email}`,
          ...
          if (!phoneValidation.valid) {
            ...
            await db.logSystem({
              ...
              source: "auth",
              action: "register_blocked",
              message: `Invalid phone rejected: ${registrationData.phone} — ${phoneValidation.error}`,
              ipAddress: getClientIp(req),
            });
            return;
          }
          ...
          const normalizedPhone =
            ...
            normalizePhone(registrationData.phone);
          ...
          if (newUser) {
            ...
            try {
              ...
              console.warn(
                "[Register] Failed to log welcome bonus history:",
                e
              );
            }
            ...
            if (registrationData.referralCode) {
              try {
                ...
                const refCode = await db.getReferralCodeByCode(
                ...
                if (refCode && refCode.userId !== newUser.id) {
                  ...
                  if (referrer) {
                    ...
                    await db.addRewardsHistory({
                      ...
                      description: `Referral bonus: ${registrationData.name || registrationData.email} joined using your code`,
                    } as any);
                  }
                  // Award referee (new user)
                  const currentPoints = 25; // already set welcome bonus above
                  await db.updateUser(newUser.id, {
                    rewardPoints: currentPoints + REFERRAL_BONUS_REFEREE,
                  ...
                  await db.addRewardsHistory({
                    ...
                    type: "referral" as any,
                    description: `Referral bonus: Used code ${registrationData.referralCode}`,
                  } as any);
                  // Track referral
                  ...
                  console.log(
                    `[Referral] ${registrationData.email} used code ${registrationData.referralCode}. Referrer +${REFERRAL_BONUS_REFERRER} pts, Referee +${REFERRAL_BONUS_REFEREE} pts`
                  );
                } else if (!refCode) {
                  console.warn(
                    `[Referral] Invalid referral code: ${registrationData.referralCode}`
                  );
                }
              } catch (refErr) {
                console.warn(
                  "[Register] Referral processing failed (non-blocking):",
                  refErr
                );
              }
            }
          ...
          const fullRegUser = createdUser
            ? await buildFullUserResponse(createdUser)
            : {
                name: fullName,
          ...
          if (registrationData.email) {
            ...
            db.logSystem({
              ...
              source: "auth",
              action: "register",
              message: `New user registered: ${registrationData.email}`,
              userId: newUser?.id,
              ipAddress: clientIp,
            }).catch(() => {});
          }
          ...
          res.json({
            success: true,
            message:
              "Account created successfully! You earned 25 welcome bonus points.",
            user: fullRegUser,
          });
        } else {
          // Login flow
          ...
          if ((user as any).isLocked) {
            res.status(403).json({
              error:
                "Your account has been locked. Please contact support at support@mylegacycannabis.ca.",
            });
            return;
          }
          ...
          await db
            .updateUser(user.id, { lastIp: loginIp } as any)
            .catch(() => {});
          await db
            .logSystem({
              level: "info",
              source: "auth",
              action: "login",
              message: `User logged in: ${user.email || user.phone}`,
              userId: user.id,
              ipAddress: loginIp,
            })
            .catch(() => {});
  ...
  app.get("/api/auth/google/callback", async (req: Request, res: Response) => {
    try {
      ...
      console.log(
        `[Google Auth] Callback received — returnTo: ${returnTo}, hasCode: ${!!code}`
      );
      ...
      if (user) {
        ...
        if (!user.phone) {
          console.log(
            `[Google Auth] Existing user ${googleUser.email} — no phone, redirecting to /complete-profile`
          );
        ...
        console.log(
          `[Google Auth] Existing user ${googleUser.email} — session set, redirecting to ${returnTo}`
        );
        ...
        console.log(
          `[Google Auth] New user ${googleUser.email} — session set, redirecting to /complete-profile`
        );
  ...
  app.post(
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        const { phone, birthday, name } = req.body as {
          phone: string;
          birthday?: string;
          name?: string;
        };
        ...
        if (birthday && !isAtLeast19(birthday)) {
          res
            .status(403)
            .json({ error: "You must be 19 years of age or older." });
          return;
        }
