...
export function registerVerifyRoutes(app: Router) {
      ...
      try {
        ...
        if (!idVerifEnabled) {
          return res.json({
            ...
            verificationId: 0,
            message:
              "ID verification is not currently required. You can place orders directly.",
            skipped: true,
          });
        }
  ...
  app.post("/api/verify/mobile-session", (req, res) => {
    const email = (req.body.email || "").trim().toLowerCase();
    const userId = req.body.userId
      ? parseInt(req.body.userId, 10) || null
      : null;
    ...
    async (req, res) => {
      try {
        ...
        if (!session || Date.now() - session.createdAt > 30 * 60 * 1000) {
          return res
            .status(400)
            .json({ error: "Session expired. Generate a new QR code." });
        }
        ...
        const email =
          (req.body.email || session.email || "").trim().toLowerCase() ||
          "guest@unknown.com";
        ...
        console.log(
          `[VERIFY] Mobile submission #${dbId} from ${email} via QR bridge`
        );
        res.json({
          success: true,
          verificationId: dbId,
          message:
            "Your ID has been submitted. You can close this page and return to your computer.",
        });
      } catch (err: any) {
