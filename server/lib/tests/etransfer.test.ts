...
describe("e-Transfer Parsing Logic", () => {
  describe("extractAmount", () => {
    it("extracts standard dollar amount", () => {
      expect(
        extractAmount(
          "Interac e-Transfer: $120.05",
          "You have received $120.05"
        )
      ).toBe(120.05);
    });
    ...
    it("handles commas in amounts", () => {
      expect(
        extractAmount(
          "Interac e-Transfer: $1,200.50",
          "You have received $1,200.50"
        )
      ).toBe(1200.5);
    });
    ...
    it("extracts CAD amounts", () => {
      expect(
        extractAmount(
          "Interac e-Transfer: 120.05 (CAD)",
          "You have received 120.05 (CAD)"
        )
      ).toBe(120.05);
    });
    ...
    it("returns null if no amount found", () => {
      expect(
        extractAmount("Interac e-Transfer", "You have received money")
      ).toBe(null);
    });
  });
  ...
  describe("extractSenderName", () => {
    it("extracts name from standard subject", () => {
      expect(
        extractSenderName("Interac e-Transfer: John Doe sent you money", "", "")
      ).toBe("John Doe");
    });
    ...
    it("extracts name from virement interac subject", () => {
      expect(
        extractSenderName(
          "Virement Interac : Jane Smith vous a envoyé de l'argent",
          "",
          ""
        )
      ).toBe("Jane Smith");
    ...
    it("extracts from email fallback", () => {
      expect(
        extractSenderName("Interac e-Transfer", "Body", "bob.builder@gmail.com")
      ).toBe("Bob Builder");
    });
  });
  ...
  describe("extractMemo", () => {
    it("extracts standard message", () => {
      expect(extractMemo("Message: Thanks for the stuff")).toBe(
        "Thanks for the stuff"
      );
    });

    it("extracts alternative message format", () => {
      expect(extractMemo("Message : Payment for order 123")).toBe(
