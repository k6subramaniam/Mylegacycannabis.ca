...
test.describe("Smart e-Transfer Matching @critical @payments @smart-match", () => {
  ...
  test.describe("Unique Cent Checkout", () => {
    ...
    test("e-Transfer checkout total has unique cents (not .00)", async ({
      page,
    }) => {
      ...
      const amountElements = page.locator(
        '[data-testid="etransfer-amount"], [class*="payment-amount"]'
      );
      ...
      if (amountMatch) {
        ...
        if (!hasUniqueCents) {
          console.warn(
            "[Smart Match] No unique cents detected — PR #110 may not be fully deployed"
          );
        }
      }
    });

    test("checkout shows mandatory acknowledgment with exact amount", async ({
      page,
    }) => {
      await page.goto("/shop");
      await page.waitForLoadState("networkidle");
  ...
  test.describe("Admin Smart Match Review", () => {
    ...
    test("admin Payments page loads with match controls", async ({ page }) => {
      ...
      const hasPayments = await page
        .getByText(/matched|unmatched|pending|payment/i)
        .first()
        .isVisible({ timeout: 5_000 })
        .catch(() => false);
    ...
    test("unmatched payments show likely match suggestions", async ({
      page,
    }) => {
      await page.goto("/admin");
      ...
      const unmatchedSection = page.getByText(
        /unmatched|needs review|smart match/i
      );
      if (
        await unmatchedSection
          .first()
          .isVisible({ timeout: 5_000 })
          .catch(() => false)
      ) {
        ...
        const matchCard = page
          .locator('[class*="match"], [data-testid*="match"]')
          .first();
        if (await matchCard.isVisible({ timeout: 3_000 }).catch(() => false)) {
          // Should have confirm/dismiss actions
          const confirmBtn = matchCard.getByRole("button", {
            name: /confirm|approve|match/i,
          });
          const dismissBtn = matchCard.getByRole("button", {
            name: /dismiss|ignore|reject/i,
          });
    ...
    test("payment reassign tool works", async ({ page }) => {
      ...
      const statusBtn = page
        .getByRole("button", { name: /reassign|change status|unmatch/i })
        .first();
      if (await statusBtn.isVisible({ timeout: 3_000 }).catch(() => false)) {
        ...
        const hasSelector = await page
          .locator(
            '[role="dialog"], [role="listbox"], [class*="dropdown"], [class*="modal"]'
          )
          .first()
    ...
    test("keyword rules configuration is accessible", async ({ page }) => {
      ...
      const rulesSection = page.getByText(
        /keyword|detection rules|matching rules/i
      );
      if (
        await rulesSection
          .first()
  ...
  test.describe("Matching API", () => {
    test("payment email endpoint returns configured email", async ({
      request,
    }) => {
