...
test.describe('Checkout & e-Transfer Flow @critical @checkout', () => {
  ...
  test('checkout page loads with cart summary', async ({ page }) => {
    ...
    await expect(page.getByText(/checkout|order summary/i).first()).toBeVisible({ timeout: 5_000 });
  });

  test('selecting e-Transfer shows payment instructions with unique cent amount', async ({ page }) => {
    ...
    const etransferOption = page.getByRole('button', { name: /e-transfer|interac/i })
      .or(page.locator('[data-testid="payment-etransfer"]'))
      .or(page.getByText(/e-transfer/i));

    if (!(await etransferOption.isVisible({ timeout: 3_000 }).catch(() => false))) {
    ...
    await expect(page.getByText(/send.*exactly/i).or(page.getByText(/e-transfer instructions/i))).toBeVisible({ timeout: 5_000 });

    // Should show an amount with cents
    const amountElement = page.locator('[data-testid="etransfer-amount"]').or(page.getByText(/\$\d+\.\d{2}/));
    await expect(amountElement.first()).toBeVisible();
  });

  test('e-Transfer checkout requires mandatory acknowledgment checkbox', async ({ page }) => {
    await page.goto('/shop');
    await page.waitForLoadState('networkidle');
    ...
    const checkbox = page.getByRole('checkbox', { name: /i understand/i })
      .or(page.locator('input[type="checkbox"]').filter({ hasText: /understand|acknowledge/i }));
  ...
  test('order confirmation shows correct e-Transfer instructions', async ({ page }) => {
    ...
    const hasInstructions = await page.getByText(/e-transfer|send.*payment/i).isVisible({ timeout: 3_000 }).catch(() => false);
    if (hasInstructions) {
...
test.describe('Cart Operations @checkout', () => {
  ...
  test('cart persists after page reload', async ({ page }) => {
    ...
    const cartIndicator = page.locator('[data-testid="cart-count"], [class*="cart-count"]').first();
