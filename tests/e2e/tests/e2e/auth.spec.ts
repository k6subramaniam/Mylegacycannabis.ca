...
test.describe('Authentication Flows @critical @auth', () => {
  ...
  test('OTP input handles paste and keyboard navigation', async ({ page }) => {
    ...
    await page.getByPlaceholder(/email/i).fill(process.env.TEST_USER_EMAIL || 'testuser@mylegacycannabis.ca');
    await page.getByRole('button', { name: /send verification code/i }).click();
  ...
  test('Google login button navigates to OAuth endpoint', async ({ page }) => {
    ...
    const navigationPromise = page.waitForURL(/accounts\.google\.com|\/api\/auth\/google/i, { timeout: 5_000 }).catch(() => null);
    await googleBtn.click();

    // Should navigate to Google or the API redirect endpoint
    const url = page.url();
    expect(url.includes('google') || url.includes('/api/auth/google')).toBeTruthy();
  });
  ...
  test('unauthenticated /account access redirects to /login without flash', async ({ page }) => {
  ...
  test('unauthenticated /admin access redirects appropriately', async ({ page }) => {
    await page.goto('/admin');

    // Should redirect to login or show unauthorized
    const url = page.url();
    const isRedirected = url.includes('/login') || url.includes('/unauthorized');
    const hasAuthPrompt = await page.getByText(/sign in|unauthorized/i).first().isVisible().catch(() => false);
