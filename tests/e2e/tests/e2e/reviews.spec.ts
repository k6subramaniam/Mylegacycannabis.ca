...
test.describe("Product Reviews @critical @reviews", () => {
  ...
  test("product page shows aggregate rating", async ({ page }) => {
    ...
    const ratingElement = page
      .locator('[class*="rating"], [data-testid*="rating"], [class*="stars"]')
      .first();
  ...
  test("review cards are collapsible (PR #45)", async ({ page }) => {
    ...
    const expandBtn = page
      .getByRole("button", { name: /show more|expand|details|read more/i })
      .first();
  ...
  test("review filter bar allows sorting and filtering", async ({ page }) => {
    ...
    if (
      ...
      const sortSelect = page
        .locator('select, [role="combobox"], [data-testid="review-sort"]')
        .first();
      ...
      const starFilter = page
        .getByRole("button", { name: /5 star|4 star|★/i })
        .first();
  ...
  test.describe("Submit Review", () => {
    ...
    test("write review form is accessible on product page", async ({
      page,
    }) => {
      await page.goto("/shop");
      await page.waitForLoadState("networkidle");
      ...
      const writeBtn = page.getByRole("button", {
        name: /write.*review|add.*review|leave.*review/i,
      });
      const reviewForm = page.locator(
        'form[class*="review"], [data-testid="review-form"]'
      );

      const hasReviewUI =
        (await writeBtn.isVisible({ timeout: 5_000 }).catch(() => false)) ||
      ...
      if (hasReviewUI && (await writeBtn.isVisible().catch(() => false))) {
        ...
        const starSelector = page
          .locator(
            '[class*="star-select"], [data-testid="star-input"], [role="radiogroup"]'
          )
          .first();
        await expect(
          starSelector.or(
            page.locator('textarea, input[name*="review"]').first()
          )
    ...
    test('delivered order shows "Write a Review" link per product (PR #47)', async ({
      page,
    }) => {
      ...
      const reviewLink = page
        .getByText(/write.*review|review/i)
        .or(page.locator('[data-testid="review-link"]'));
      if (
        await reviewLink
          .first()
          .isVisible({ timeout: 5_000 })
  ...
  test.describe("Admin Moderation", () => {
    ...
    test("admin Reviews page lists reviews with moderation controls", async ({
      page,
    }) => {
      ...
      const reviewsLink = page
        ...
        .or(page.getByRole("button", { name: /reviews/i }));
      if (
        !(await reviewsLink.isVisible({ timeout: 3_000 }).catch(() => false))
      ) {
      ...
      const moderationBtn = page
        .getByRole("button", { name: /approve|delete|edit|moderate/i })
        .first();
      if (
        await moderationBtn.isVisible({ timeout: 3_000 }).catch(() => false)
      ) {
  ...
  test("product page has AggregateRating structured data", async ({ page }) => {
    ...
    const hasRating = jsonLd.some(
      ld => ld.includes("AggregateRating") || ld.includes("aggregateRating")
    );
