...
test.describe('Maintenance Mode & Store Hours @admin @maintenance', () => {
  ...
  test.describe('Admin Maintenance Controls', () => {
    ...
    test('admin Settings has maintenance mode toggle', async ({ page }) => {
      ...
      const toggle = page.locator('input[type="checkbox"], [role="switch"]').filter({ hasText: /maintenance/i }).or(
        page.locator('[data-testid*="maintenance"]')
      );
    ...
    test('admin Settings has store hours configuration', async ({ page }) => {
      ...
      if (await hoursSection.first().isVisible({ timeout: 5_000 }).catch(() => false)) {
        await expect(hoursSection.first()).toBeVisible();
      }
    });

    test('maintenance overlay has logo and location carousel (PR #11)', async ({ page }) => {
      ...
      const msgField = page.locator('textarea, input').filter({ hasText: /maintenance|message/i });
      // Just verify settings page has maintenance controls
    });
  });

  test('footer conditionally renders store hours column (PR #74)', async ({ page }) => {
    await page.goto('/');
    ...
    const emptyDivCount = (footerHtml.match(/<div[^>]*>\s*<\/div>/g) || []).length;
