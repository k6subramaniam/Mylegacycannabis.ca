...
test.describe("AI Features @admin @ai", () => {
  ...
  test("admin Settings has AI configuration section", async ({ page }) => {
    ...
    const aiSection = page.getByText(
      /ai.*config|ai.*provider|ai.*settings|openai|gemini/i
    );
    await expect(aiSection.first()).toBeVisible({ timeout: 5_000 });
  });

  test("AI config supports dual-provider (primary + fallback) (PR #97)", async ({
    page,
  }) => {
    await page.goto("/admin");
    const settingsLink = page
    ...
    const modelSelect = page
      ...
      .filter({ hasText: /gpt|gemini|model/i });
  ...
  test("AI config has test connection button", async ({ page }) => {
    ...
    const testBtn = page.getByRole("button", {
      name: /test.*connection|test.*ai|verify.*key/i,
    });
    if (
      await testBtn
        .first()
  ...
  test("admin Products has AI menu import tool", async ({ page }) => {
    ...
    const importBtn = page.getByRole("button", {
      name: /import|menu.*import|ai.*import/i,
    });
    if (
      await importBtn
        .first()
  ...
  test("admin Insights has AI user profiles section", async ({ page }) => {
    ...
    if (
      !(await insightsLink.isVisible({ timeout: 3_000 }).catch(() => false))
    ) {
      test.skip(true, "Insights not visible");
      return;
    }
    ...
    // Behavior & AI tab
    const behaviorTab = page
      .getByRole("button", { name: /behavior|ai/i })
      .first();
    if (await behaviorTab.isVisible({ timeout: 3_000 }).catch(() => false)) {
      ...
      const profilesSection = page.getByText(
        /ai.*profile|user.*profile|behavior.*profile/i
      );
      if (
        await profilesSection
          .first()
          .isVisible({ timeout: 5_000 })
          .catch(() => false)
      ) {
        ...
        const refreshBtn = page.getByRole("button", {
          name: /refresh.*all|refresh.*profiles/i,
        });
