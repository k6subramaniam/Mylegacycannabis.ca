...
test.describe("Email System @admin @email", () => {
  ...
  test("admin Email Templates page lists all templates", async ({ page }) => {
    ...
    if (!(await emailLink.isVisible({ timeout: 3_000 }).catch(() => false))) {
      test.skip(true, "Email section not visible");
      return;
    }
  ...
  test("email template editor has visual and code modes (PR #67)", async ({
    page,
  }) => {
    await page.goto("/admin");
    const emailLink = page
    ...
    const firstTemplate = page
      .locator('[class*="template-card"], [data-testid*="template"]')
      .first();
  ...
  test("email template supports AI generation (PR #52)", async ({ page }) => {
    ...
    const aiBtn = page.getByRole("button", {
      name: /ai.*generate|generate.*ai|improve.*ai|ai.*improve/i,
    });
    if (
      await aiBtn
        .first()
  ...
  test("email template has variable insertion tools (PR #67)", async ({
    page,
  }) => {
    await page.goto("/admin");
    const emailLink = page
    ...
    if (await firstTemplate.isVisible({ timeout: 5_000 }).catch(() => false)) {
      ...
      const varBtn = page.getByRole("button", {
        name: /variable|insert|snippet|\{\{/i,
      });
      if (
        await varBtn
          .first()
  ...
  test("email health monitor shows provider status", async ({ page }) => {
    ...
    const healthSection = page.getByText(
      /health|provider|delivery|resend|smtp/i
    );
    if (
      await healthSection
        .first()
  ...
  test("apply design button exists for cross-template styling", async ({
    page,
  }) => {
    await page.goto("/admin");
    const emailLink = page
    ...
    const applyBtn = page.getByRole("button", {
      name: /apply.*design|copy.*design/i,
    });
    if (
      await applyBtn
        .first()
