...
test.describe('French/English Translation @i18n @ux', () => {
  ...
  test('FR/EN toggle is visible in the header', async ({ page }) => {
    ...
    const langToggle = page.getByRole('button', { name: /fr|en|français|english/i })
  ...
  test('clicking FR toggle switches page to French', async ({ page }) => {
    ...
    // Click FR toggle
    const frToggle = page.locator('button:has-text("FR"), [data-testid="lang-fr"]').first();
  ...
  test('switching back to EN restores English', async ({ page }) => {
    ...
    // Switch back to EN
    const enToggle = page.locator('button:has-text("EN"), [data-testid="lang-en"]').first();
  ...
  test('language preference persists across page navigation', async ({ page }) => {
    ...
    const bodyText = await page.textContent('body');
    const hasFrench = /magasin|boutique|panier|produits|filtres/i.test(bodyText || '');
  ...
  test.skip('shop page renders correctly in French', async ({ page }) => {
    ...
    const products = page.locator('[class*="product-card"], [data-testid="product-card"]');
  ...
  test.skip('login page renders correctly in French', async ({ page }) => {
    ...
    const bodyText = await page.textContent('body');
    const hasFrenchAuth = /connexion|se connecter|courriel|mot de passe|créer|login|email|password/i.test(bodyText || '');
  ...
  test('pages have hreflang tags for en-CA and fr-CA', async ({ page }) => {
    ...
    const hreflangEn = page.locator('link[hreflang="en-CA"], link[hreflang="en-ca"], link[hreflang="en"]');
    const hreflangFr = page.locator('link[hreflang="fr-CA"], link[hreflang="fr-ca"], link[hreflang="fr"]');
