...
test.describe('API Endpoints @api', () => {
  ...
  test('GET /api/auth/google-available returns boolean', async ({ request }) => {
  ...
  test('GET /api/auth/smtp-available returns provider info', async ({ request }) => {
  ...
  test('POST /api/auth/verify-otp rejects invalid code', async ({ request }) => {
  ...
  test('GET /api/auth/google redirects to Google OAuth', async ({ request }) => {
  ...
  test('GET /api/geo/nearest-store returns valid structure', async ({ request }) => {
  ...
  test('FreeIPAPI resolves Canadian IP correctly', async ({ request }) => {
    ...
    const res = await request.get('https://freeipapi.com/api/json/99.228.100.1', {
      timeout: 5_000,
    });
    expect(res.ok()).toBeTruthy();
    const data = await res.json();
  ...
  test('auth endpoints have rate limiting', async ({ request }) => {
    ...
    const promises = Array.from({ length: 10 }, () =>
      request.post('/api/auth/send-otp', {
        data: { identifier: 'ratetest@test.com', type: 'email', purpose: 'login' },
      })
    );
  ...
  test('analytics respects Do Not Track header', async ({ request }) => {
    ...
    if (res.ok() && res.headers()['content-type']?.includes('application/json')) {
      const data = await res.json();
  ...
  test('analytics opt-out cookie endpoint works', async ({ request }) => {
    ...
    if (optOutRes.ok() && optOutRes.headers()['content-type']?.includes('application/json')) {
    ...
    if (optInRes.ok() && optInRes.headers()['content-type']?.includes('application/json')) {
