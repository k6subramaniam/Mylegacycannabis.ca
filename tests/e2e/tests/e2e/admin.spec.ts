...
test.describe('Admin Dashboard @admin', () => {
  ...
  test('admin page loads with navigation tabs', async ({ page }) => {
    ...
    await expect(page.getByText(/admin/i).first()).toBeVisible({ timeout: 5_000 });
  });
  ...
  test('Insights page has Behavior & Geo-Analytics tabs', async ({ page }) => {
    ...
    await expect(page.getByText(/behavior.*ai/i).or(page.getByRole('button', { name: /behavior/i }))).toBeVisible({ timeout: 5_000 });
    await expect(page.getByText(/geo-analytics/i).or(page.getByRole('button', { name: /geo/i }))).toBeVisible();
  });
  ...
  test('Geo-Analytics tab renders KPI cards', async ({ page }) => {
    ...
    const geoTab = page.getByRole('button', { name: /geo-analytics/i }).or(page.getByText(/geo-analytics/i));
    await geoTab.click();

    // KPI cards should render
    await expect(page.getByText(/total events/i)).toBeVisible({ timeout: 10_000 });
  ...
  test('Geo-Analytics province map renders', async ({ page }) => {
    ...
    const mapSection = page.getByText(/traffic by province/i)
  ...
  test('Geo-Analytics period selector changes data', async ({ page }) => {
    ...
    if (await sevenDay.isVisible({ timeout: 3_000 }).catch(() => false)) {
      ...
      await expect(page.getByText(/error|failed/i)).not.toBeVisible({ timeout: 3_000 }).catch(() => {});
    }
  ...
  test('Geo-Analytics city table displays data', async ({ page }) => {
    ...
    const citySection = page.getByText(/top cities/i)
    ...
    const hasCityData = await page.getByText(/montreal|toronto/i).first().isVisible({ timeout: 5_000 }).catch(() => false);
  ...
  test('admin orders page lists orders', async ({ page }) => {
    ...
    if (await ordersLink.isVisible({ timeout: 3_000 }).catch(() => false)) {
      ...
      await expect(page.getByText(/error|failed to load/i)).not.toBeVisible({ timeout: 3_000 }).catch(() => {});
    }
  });
...
test.describe('Admin Account Page @admin', () => {
  ...
  test('authenticated user sees account page with profile data', async ({ page }) => {
  ...
  test('rewards tab shows points balance and referral code', async ({ page }) => {
    await page.goto('/account');
    ...
    // Points balance
    await expect(page.getByText(/available points|points/i)).toBeVisible({ timeout: 5_000 });
