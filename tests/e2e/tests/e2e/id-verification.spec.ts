...
test.describe('ID Verification @critical @verification', () => {
  ...
  test.describe('Customer Verification UI', () => {
    ...
    test('account page shows ID verification status', async ({ page }) => {
      ...
      const verifySection = page.getByText(/id verification|verification status|verified|not verified|pending/i);
      if (await verifySection.first().isVisible({ timeout: 5_000 }).catch(() => false)) {
        await expect(verifySection.first()).toBeVisible();

        // Should show status badge (verified/pending/not verified)
        const statusBadge = page.locator('[class*="verified"], [class*="pending"], [data-testid*="verify-status"]');
        if (await statusBadge.first().isVisible({ timeout: 2_000 }).catch(() => false)) {
    ...
    test('verify now button navigates to upload page', async ({ page }) => {
      ...
      const verifyBtn = page.getByRole('button', { name: /verify now|check status/i })
      ...
      if (await verifyBtn.isVisible({ timeout: 5_000 }).catch(() => false)) {
        ...
        const onVerifyPage = page.url().includes('verify') ||
          await page.getByText(/upload.*id|photo.*id|government.*id/i).first().isVisible({ timeout: 5_000 }).catch(() => false);
      }
    });
    ...
    test('ID upload page accepts image files', async ({ page }) => {
      ...
      const instructions = page.getByText(/front.*id|government.*issued|driver|passport/i);
      if (await instructions.first().isVisible({ timeout: 3_000 }).catch(() => false)) {
  ...
  test.describe('Admin Verification Panel', () => {
    ...
    test('admin has ID verification review section', async ({ page }) => {
      ...
      const verifyLink = page.getByRole('link', { name: /verification|verify|customers/i }).first();
      if (await verifyLink.isVisible({ timeout: 3_000 }).catch(() => false)) {
        ...
        const hasVerifyUI = await page.getByText(/pending.*review|verification|approve|reject/i).first().isVisible({ timeout: 5_000 }).catch(() => false);
      }
    });

    test('admin Settings has ID verification toggle (PR #9)', async ({ page }) => {
      await page.goto('/admin');
      ...
      const verifyToggle = page.getByText(/id verification|require.*verification/i);
      if (await verifyToggle.first().isVisible({ timeout: 5_000 }).catch(() => false)) {
    ...
    test('admin has AI verification toggle (PR #56)', async ({ page }) => {
      ...
      if (await aiToggle.first().isVisible({ timeout: 5_000 }).catch(() => false)) {
