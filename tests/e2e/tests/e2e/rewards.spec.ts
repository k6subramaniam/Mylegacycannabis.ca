...
test.describe('Rewards System @critical @rewards', () => {
  ...
  test('account rewards tab shows points balance', async ({ page }) => {
    ...
    const rewardsTab = page.getByRole('button', { name: /rewards|my rewards/i });
    ...
    const balanceText = await page.locator('[class*="points"], [data-testid*="points"]').first().textContent().catch(() => null);
    if (balanceText) {
  ...
  test('header shows reward points for authenticated user', async ({ page }) => {
    await page.goto('/');

    // PR #90: rewards points display in header
    const headerPoints = page.locator('[data-testid="header-points"], [class*="reward-badge"]');
  ...
  test('rewards tab shows referral code and copy button', async ({ page }) => {
    ...
    // Referral section
    await expect(page.getByText(/refer a friend/i)).toBeVisible({ timeout: 5_000 });

    // Referral code should be visible (format: MLC-XXXX)
    const codeElement = page.locator('[class*="referral-code"], [data-testid="referral-code"], [class*="mono"]').first();
  ...
  test('referral share link button exists', async ({ page }) => {
    ...
    // Share button (PR #48)
    const shareBtn = page.getByRole('button', { name: /share/i }).or(page.locator('[data-testid="referral-share"]'));
    if (await shareBtn.isVisible({ timeout: 3_000 }).catch(() => false)) {
      await expect(shareBtn).toBeVisible();
    }
  });

  test('registration page accepts referral code via URL param', async ({ page }) => {
    ...
    const refInput = page.locator('input[name*="referral"], input[placeholder*="referral"]');
    ...
    } else if (await refSection.isVisible({ timeout: 2_000 }).catch(() => false)) {
  ...
  test('checkout shows points to be earned', async ({ page }) => {
    ...
    const pointsEarned = page.getByText(/earn.*point|point.*earn|\d+\s*points/i);
    if (await pointsEarned.first().isVisible({ timeout: 5_000 }).catch(() => false)) {
  ...
  test('public rewards page explains the program', async ({ page }) => {
    ...
    await expect(page.getByText(/rewards|loyalty/i).first()).toBeVisible({ timeout: 5_000 });
