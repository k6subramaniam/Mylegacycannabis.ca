...
test.describe("Production Smoke Tests @smoke", () => {
  test("homepage loads with logo and age gate", async ({ page }) => {
    ...
    const hasAgeGate = await page
      .locator(
        '[data-testid="age-gate"], [class*="age-gate"], [class*="ageGate"]'
      )
      .isVisible()
      .catch(() => false);
    const hasMainContent = await page
      .locator("text=/shop|featured|products/i")
      .first()
      .isVisible()
      .catch(() => false);
  ...
  test("shop page loads with products", async ({ page }) => {
    ...
    await expect(
      page.locator('[class*="product"], [data-testid*="product"]').first()
  ...
  test("mobile viewport renders correctly", async ({ page }) => {
    ...
    const menuBtn = page
      .locator('[class*="hamburger"], button[aria-label*="menu"], .menu-toggle')
      .first();
