...
test.describe("Image Upload & File Persistence @admin @images @files", () => {
  ...
  test("admin Products editor has drag-and-drop image uploader", async ({
    page,
  }) => {
    ...
    const productRow = page
      .locator('tr, [class*="product-row"], [data-testid*="product"]')
      .first();
    ...
    if (await editBtn.isVisible({ timeout: 5_000 }).catch(() => false)) {
      ...
      const uploadArea = page.locator(
        '[class*="dropzone"], [class*="drag-drop"], [class*="image-upload"], input[type="file"]'
      );
      if (
        await uploadArea
          .first()
  ...
  test("admin Settings has logo upload section", async ({ page }) => {
    ...
    const uploadBtn = page.getByRole("button", {
      name: /upload.*logo|change.*logo/i,
    });
    const hasUpload =
      (await uploadInput.isVisible({ timeout: 2_000 }).catch(() => false)) ||
  ...
  test("logo has dimension tooltip guidance (PR #66)", async ({ page }) => {
    ...
    const guidance = page.getByText(
      /recommended.*dimension|png|transparent|pixel/i
    );
    if (
      await guidance
        .first()
  ...
  test("logo renders with cache-busting URL (PR #73)", async ({ page }) => {
    ...
    const logo = page
      .locator('header img[alt*="legacy" i], header img[src*="logo"]')
      .first();
  ...
  test("uploaded images are accessible after page reload", async ({ page }) => {
    ...
    const productImg = page
      .locator('[class*="product-card"] img, [data-testid="product-card"] img')
      .first();
    if (await productImg.isVisible({ timeout: 5_000 }).catch(() => false)) {
      const naturalWidth = await productImg.evaluate(
        (img: HTMLImageElement) => img.naturalWidth
      );
