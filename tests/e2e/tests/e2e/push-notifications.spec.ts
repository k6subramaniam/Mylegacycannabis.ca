...
test.describe('PWA & Push Notifications @critical @pwa @push', () => {
  ...
  test('service worker registers successfully', async ({ page }) => {
    ...
    if (!swRegistered) {
      console.warn('[PWA] Service worker not registered — may need second visit');
    }
  });
  ...
  test('push config endpoint returns VAPID public key', async ({ request }) => {
    ...
    const trpcRes = await request.get('/api/trpc/push.getConfig').catch(() => null);
    ...
    const hasKey = JSON.stringify(data).includes('BN') || JSON.stringify(data).includes('vapid');
    if (!hasKey) {
      console.warn('[Push] VAPID key not found in config response — env vars may not be set');
    }
  });
  ...
  test.describe('Push Opt-In', () => {
    ...
    test('push opt-in banner appears for authenticated users', async ({ page }) => {
      await page.goto('/');
      await page.waitForLoadState('networkidle');

      // The opt-in banner should appear after certain triggers
      // (after ID verification or first purchase per PR #94)
      const optInBanner = page.locator('[data-testid="push-opt-in"], [class*="push-banner"], [class*="push-opt"]');
      const enableBtn = page.getByRole('button', { name: /enable.*notif|turn on.*notif|allow.*notif/i });
      ...
      if (await optInBanner.isVisible({ timeout: 3_000 }).catch(() => false)) {
        ...
        const dismissBtn = page.getByRole('button', { name: /not now|dismiss|later|no thanks/i });
    ...
    test('dismissed push banner stays hidden for the session', async ({ page }) => {
      await page.goto('/');
      await page.waitForLoadState('networkidle');

      const optInBanner = page.locator('[data-testid="push-opt-in"], [class*="push-banner"]');
      const dismissBtn = page.getByRole('button', { name: /not now|dismiss|later/i });
      ...
      await expect(optInBanner).not.toBeVisible({ timeout: 2_000 }).catch(() => {});
    });
  });
  ...
  test.describe('Admin Push Management', () => {
    ...
    test('admin Settings shows push notification section', async ({ page }) => {
      ...
      if (await pushSection.first().isVisible({ timeout: 5_000 }).catch(() => false)) {
        ...
        const bodyInput = page.locator('textarea, input[placeholder*="message"], input[name*="body"]').first();
    ...
    test('push broadcast shows recent notification log', async ({ page }) => {
      ...
      const logSection = page.getByText(/recent.*notif|notification.*log|push.*log/i);
      if (await logSection.first().isVisible({ timeout: 5_000 }).catch(() => false)) {
